...
var setting = {
		...
		var paramtabs_map = {Node: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"检查点")%>', url:'<%= basePath%>form/toForm.action?formId=4357'}]}//4419		
		...
		function onAsyncSuccess(event, treeId, treeNode){
				...
				$("#main").width(iFrameWidth+"px");
						document.getElementById("main").src = setUrlParam(paramtabs_map["Node"][0].url, nodes[0].children[0]);
				}
