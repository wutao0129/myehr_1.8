	...
	<div style="margin-top:5px;">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="editorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"选择")%> onclick="selectFieldRule()"/>
			</div>
		</div>
...
function selectFieldRule(){
	...
	var param  = {};
	var fieldRuleId = select.FIELD_RULE_ID;
	var fieldRuleName = select.FIELD_RULE_NAME;
	var fieldId = '${param.fieldId}';
	var selectData={fieldRuleId:fieldRuleId,fieldRuleName:fieldRuleName,fieldId:fieldId};
	param.selectData=selectData;
	
	$.ajax({
		url:'${pageContext.request.contextPath }/FieldList/updateFieldWithFieldRule.action',
		type:'post',
		cache: false,
		contentType: 'application/json;charset=utf-8',
