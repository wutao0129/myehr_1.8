...
<div class="container-fluid" style="width:100%;height:100%;padding:1% 0">
	...
	<div  style="width:75%;height:4%;margin:auto auto;">
		<div class="BTNGROUP BTNGROUP_1042" style="margin:5px auto; display:inline-block;height: 30px;">
			...
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addElement"  class="btn btn-info" value="增加" onclick="addElement()"/>
			</div>
		</div>
	</div>
		...
		<div class="row cell" id="divSize">
			...
			<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
				<div class="ui search selection dropdown entitybox field-control" style="display: inline-block;">
					<lable style="float:left;font-size:12px">数据显示列数: </lable>
...
function initSelect(){
	...
	var objs = $("select");
	for(var k=cesllObjNum;k<objs.length;k++){
		for(var i=0;i<data.length;i++){
			document.getElementById(objs.eq(k).attr("id")).options.add(new Option(data[i].text,data[i].id));		
		}
	}
...
function yulan(){
	...
	for(var i=0;i<rows.length;i++){
		...
		if(rows[i].id=='divSize'){
			...
			sysGridbycardTemplateColumn.templateColumnValue=$("#large").val();
	...
	var right1s_code = "";
	var model ="";
	var large = sList[0].templateColumnValue;
	var colors = $("#backgroundColor").val();
	var numElement = 12/large;
	for(var i = 0 ;i<4;i++){
		 colorNum = i%4;
		 model+='<div class="col-md-12 col-sm-12 col-lg-12">'+
				...
				for(var j=1;j<sList.length;j++){
					model+= '				<div class="col-lg-'+numElement+' col-md-'+numElement+' col-sm-'+numElement+' col-xs-'+numElement+'">'+
	...
	for(var i=0;i<rows.length;i++){
		...
		if(rows[i].id=='divSize'){
			...
			if(columnValue!=null&&columnValue!=''){
				...
				sysGridbycardTemplateColumn.templateColumnTemplateId=${param.templateId};
				sysGridbycardTemplateColumn.templateColumnCode=rows[i].id;
				sysGridbycardTemplateColumn.templateColumnValue=columnValue;
				sysGridbycardTemplateColumn.templateColumnFontSize=$(rows[i]).find('input[name="fontSize"]').val();
				sysGridbycardTemplateColumn.templateColumnFontWeight=$(rows[i]).find('input[name="fontWeight"]').val();
				sysGridbycardTemplateColumn.templateColumnFontColor=$(rows[i]).find('input[name="fontColor"]').val();
				sysGridbycardTemplateColumn.templateColumnFormDefId=${param.formDefId};
	    ...
	    success:function(datas){   //此函数在服务器执行成功时被调用，参数result是服务器返回的值  
			for(var k=0;k<datas.length;k++){
				if(datas[k].templateColumnCode=='divSize'){
					...
					addElement(k-1);
					var objs = $(".elementColumn");
					objs.eq(k-1).find('input[name="id"]').val(datas[k].templateColumnId);
					if(datas[k].templateColumnValue!=null){
						objs.eq(k-1).find('select[name="columnValue"]').val(datas[k].templateColumnValue.split("&"));
					}
					objs.eq(k-1).find('input[name="fontSize"]').val(datas[k].templateColumnFontSize);
					objs.eq(k-1).find('input[name="fontWeight"]').val(datas[k].templateColumnFontWeight);
					objs.eq(k-1).find('input[name="fontColor"]').val(datas[k].templateColumnFontColor);
					objs.eq(k-1).find('input[name="linkMark"]').val(datas[k].templateColumnLinkMark);
					objs.eq(k-1).find('input[name="linkMarkName"]').val(getFormNameById(datas[k].templateColumnLinkMark));
				}
			}
	    }  
    });
...
function addElement(i){
	...
	var cellobj =   '<div class="row cell elementColumn" id="right'+i+'" >\n'+
					'	<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">\n'+
					'		<div class="ui search selection dropdown entitybox field-control bs-docs-example" style="float:left">\n'+
					'			<lable style="float:left;font-size:12px">字段元素: </lable>\n'+
					'			<select id="right'+i+'_columnValue" title="" styleType="select" name="columnValue"  class="form-control" style="width:200px;" >\n'+
					'			</select>\n'+
					'			<input id="right'+i+'_id" name="id" type="hidden"/>\n'+
					'		</div>\n'+
					'	</div>\n'+
					'	<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">\n'+
					'		<div class="ui search selection dropdown entitybox field-control bs-docs-example" style="float:left">\n'+
					'			<lable style="float:left;font-size:12px">链接表单: </lable>\n'+
					'			<input id="right'+i+'_linkMark" name="linkMark" type="hidden" class="form-control"  style="width:220px;" value=""/>\n'+
					'			<input id="right'+i+'_linkMarkName" name="linkMarkName" type="text" class="form-control"  style="width:150px;float:left"/>\n'+
					'			<input type="button" id="chooseFormStep2"  class="btn btn-success" style="float:left" value="跳转表单" onclick="chooseFormStep2('+i+')"/>\n'+
					'		</div>\n'+
					'	</div>\n'+
					'	<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">\n'+
					'		<div class="ui search selection dropdown entitybox field-control bs-docs-example" style="float:left">\n'+
					'			<lable style="float:left;font-size:12px">字体大小: </lable>\n'+
					'			<input id="right'+i+'_fontSize" name="fontSize" type="number" class="form-control"  style="width:220px;float:left" value="12"/><span style="float:left">px</span>\n'+
					'		</div>\n'+
					'	</div>\n'+
					'	<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">\n'+
					'		<div class="ui search selection dropdown entitybox field-control bs-docs-example" style="float:left">\n'+
					'			<lable style="float:left;font-size:12px">字体粗细: </lable>\n'+
					'			<input id="right'+i+'_fontWeight" name="fontWeight" type="number" class="form-control"  style="width:220px;float:left" value="400"/><span style="float:left"></span>\n'+
					'		</div>\n'+
					'	</div>\n'+
					'	<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">\n'+
					'		<div class="ui search selection dropdown entitybox field-control bs-docs-example" style="float:left">\n'+
					'			<lable style="float:left;font-size:12px">字体颜色: </lable>\n'+
					'			<input id="right'+i+'_fontColor" name="fontColor" type="color" class="form-control"  style="width:220px;" value="#ffffff"/>\n'+
					'		</div>\n'+
					'	</div>\n'+
					'</div>\n';
...
function chooseFormStep2(e){
	var url = '/myehr/jsp/form/formcreate/formChoose.jsp?fromType=config_form&widget=model4&elementNum='+e;
	layer.open({
		type:2,
		closeBtn:1,
		shadeClose:true,
