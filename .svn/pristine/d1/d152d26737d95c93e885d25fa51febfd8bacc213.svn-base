...
<div class="container-fluid" style="height:100%;overflow-y:auto;">
	<div style="margin-top:5px;">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			...
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="editorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"修改字典项")%> onclick="editDictEntry()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
...
function addScheme(){
	...
	var url='/myehr/jsp/dict/editDictType.jsp?nodeId='+nodeId;
	
	parent.layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"增加字典类")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function editScheme(){
	...
	var url='/myehr/jsp/dict/editDictType.jsp?dictTypeId='+select.dictTypeId;
	
	layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"修改字典类")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
