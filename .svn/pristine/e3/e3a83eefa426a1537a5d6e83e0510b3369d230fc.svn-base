...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/editorModel/buildEditorModelTree.action?params={s_userId:userId}",//ajax的请求地址
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案列表")%>', url:'<%= basePath%>jsp/form/editorModel/selectEditorModelList.jsp'}],
...
var ORG_ABBREVIATION = node.ORG_ABBREVIATION ? node.ORG_ABBREVIATION.toLowerCase() : "";
	...
	function addNode(folderTreeParentId) {
		...
		layer.open({
			...
			yes: function(index, layero){
				...
				var url = "/myehr/editorModel/addEditorModelFolder.action";//更新树节点信息
				$.ajax({
					url:url,
					type:'post',
					cache: false,
	...
	function deleteNode(folderTreeId,folderTreeName) {
		...
		layer.msg('确定要删除\"'+folderTreeName+'\"节点', {
			  ...
			  ,yes: function(index){
				...
				$.ajax({
			        url: "/myehr/editorModel/deleteEditorModelFolder.action?folderTreeId="+folderTreeId,
			        type: 'POST',
			        cache: false,
			        async: false,
	...
	function transmit(modelId,modelName){
		var modelId1='${param.modelId}';
		var modelName1='${param.modelName}';

		window.parent.document.getElementById(modelId1).value=modelId;
		window.parent.document.getElementById(modelName1).value=modelName;
		window.parent.initModel(modelId);
		closex();
	}
