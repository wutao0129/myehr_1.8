...
<div class="container-fluid">
	<div style="margin-top:5px;">
		<input id="SYS_SYSTEM_SCHEME.dictEntryId" name="dictEntryId" type="hidden" class="form-control"  style="width:200px;"/>
		<input id="SYS_SYSTEM_SCHEME.dictTypeId" name="dictTypeId" type="hidden" class="form-control"  style="width:200px;"/>
		...
		<h3 style=" background:#FFF;color:blue; display:block;font-size:12;position:absolute; left:10px; top:-27px;text-align:center"></h3>
			...
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"字典项编码")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_SYSTEM_SCHEME.dictEntryCode" name="dictEntryCode" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
				
			</tr>
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"字典项名称")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_SYSTEM_SCHEME.dictEntryName" name="dictEntryName" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
			</tr>
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"字典项排序")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_SYSTEM_SCHEME.dictEntrySort" name="dictEntrySort" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
			</tr>
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"字典项描述")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_SYSTEM_SCHEME.dictEntryRemark" name="dictEntryRemark" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
			</tr>
		</table>
	</div>
...
$(function () {  
	...
	window.onload=function(){
		...
		document.getElementById("SYS_SYSTEM_SCHEME.dictTypeId").value="${param.dictTypeId}";
		document.getElementById("SYS_SYSTEM_SCHEME.dictEntryId").value="${param.dictEntryId}";
		var dictEntryId = '${param.dictEntryId}';
		
		if(dictEntryId!='' && dictEntryId!=null){
			_initData(dictEntryId);
		}
	...
	function save_formemp_emp_edit_click_1881(){
		var paramsMap = {'userId' : '${sessionScope.userid}','dictTypeId':'${param.dictTypeId}','dictEntryCode':'${param.dictEntryCode}','nodeType':'${param.nodeType}','nodeId':'${param.nodeId}'};
		var _param = {};
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/dictType/saveDictEntry.action',
			type:'post',
	...
	function _initData(pkId){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/dictType/queryDictEntryById.action?dictEntryId='+dictEntryId,
			type:'post',
			//data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
