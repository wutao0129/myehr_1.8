				...
				<div class="row cell" style="border-left:0px">
					<div id="Tree" class="nui-layout" style="height: 100%;position:relative;float:left;width: 17%;margin:0;">
						<div style="height:100%; border:1px solid #ccc; position:relative;border-radius:5px;margin-bottom:10px;padding:10px">
							...
							<div title="实体树" region="west" style="display:block;height:100%;margin-bottom:1%"   showHeader="false" showFilter="false" class="sub-sidebar" allowResize="false">
...
$(function () {  
	...
	window.onload=function(){
		...
		document.getElementById("SYS_SYSTEM_SCHEME.execSqlRelaid").value="${param.datafieldId}";
		document.getElementById("SYS_SYSTEM_SCHEME.execSqlType").value="${param.execSqlType}";
		var execSqlRelaid = '${param.datafieldId}';
	...
	function save_formemp_emp_edit_click_1881(){
		...
		_param.buttonId='${param.datafieldId}';
	...
	function _initData(pkId){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/button/queryExecSql.action?execSqlRelaid='+execSqlRelaid+'&execSqlType='+'${param.execSqlType}',
			type:'post',
			//data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
