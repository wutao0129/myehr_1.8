 ...
 *
 */
public class SysSolutionCacheCopy implements Serializable {
	/**
	 * 
	 */
	...
	//关联方案对象
	private List<SysRuleCacheCopy> rules = new ArrayList<SysRuleCacheCopy>();

	public List<SysRuleCacheCopy> getRules() {
		return rules;
	}

	public void setRules(List<SysRuleCacheCopy> rules) {
		this.rules = rules;
	}
	 ...
	 *
	 */
	public SysSolutionCacheCopy(){};
	
	/**
	 * 造函数通过方案ID
	 *
	 */
	public SysSolutionCacheCopy(long solutionId) throws Exception{
	 ...
	 *
	 */
	public SysSolutionCacheCopy(QSysformCalculateSolution sobj) {
		...
		for(int i=0; i<ruless.size(); i++){
			rules.add(new SysRuleCacheCopy(ruless.get(i),this));
		}
	}
