...
var setting = {
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案列表")%>', url:'<%= basePath%>jsp/form/tree/treeSolutionListSelect.jsp'}],
...
var ORG_ABBREVIATION = node.ORG_ABBREVIATION ? node.ORG_ABBREVIATION.toLowerCase() : "";
	...
	function transmit(formID,formName){
		parent.document.getElementById("SYS_ICON.formTreeSolutionId").value=formID;
		parent.document.getElementById("SYS_ICON.formTreeSolutionName").value=formName;
		closex();
	}
	</script>
</html>
