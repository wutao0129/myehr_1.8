...
function choose(){
	...
	if(type=='dictNameColumn'){
		title+=selects[0].formEntityTablename+'.'+selects[0].formFieldTablename;
		parent.$("#comboboxDictNameColumn").val(title);
	}else{
		for(var i=0;i<selects.length;i++){
			title+='['+selects[i].formEntityTablename+'.'+selects[i].formFieldTablename+']+';
		}
		title = title.substring(0,title.length-1);
		parent.$("#formDefName").val(title);
	}
	close();
}
