...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"表单目录树")%>" region="west" style="float:left;overflow:auto;width:17%;height:100%"
		showFilter="false"
	...
	<div id="iframe" title="center" region="center" style="border:0;width:83%;float:left;height:100%;">
...
function setUrlParam(url,params){
	...
	function transmit(formID,formName){
		if('${param.widget}'=='textlink'){
			parent.$("#textlinkDetailFormId").val(formID);
			parent.$("#textlinkDetailFormName").val(formName);
		}if('${param.widget}'=='mstTab'){
			parent.$("#mstTabMainFormId").val(formID);
			parent.$("#mstTabMainFormName").val(formName);
		}if('${param.widget}'=='lookup'){
			parent.$("#lookupDetailFormId").val(formID);
			parent.$("#lookupDetailFormName").val(formName);
		}if('${param.widget}'=='charts'){
			parent.$("#formChartsSelectFormId").val(formID);
			parent.$("#formChartsSelectFormName").val(formName);
		}if('${param.widget}'=='bootout'){
			parent.$("#bootoutFormId").val(formID);
			parent.$("#bootoutFormName").val(formName);
		}if('${param.widget}'=='fileupload'){
			parent.$("#fileuploadDataFromValue").val(formID);
			parent.$("#fileuploadDataFromType").val(formName);
		}if('${param.widget}'=='model3'){
			parent.$("#right1_linkMark").val(formID);
			parent.$("#right1_linkMarkName").val(formName);
		}if('${param.widget}'=='model4'){
			parent.$("#right${param.elementNum}_linkMark").val(formID);
			parent.$("#right${param.elementNum}_linkMarkName").val(formName);
		}if('${param.widget}'=='model5'){
			parent.$("#divSize_linkMark").val(formID);
			parent.$("#divSize_linkMarkName").val(formName);
		}if('${param.widget}'=='model1'){
			parent.$("#divSize_linkMark").val(formID);
			parent.$("#divSize_linkMarkName").val(formName);
		}else{
			parent.$("#tabDetailFormId").val(formID);
			parent.$("#tabDetailFormName").val(formName);
			parent.$("#tabDetailFormTitle").val(formName);
		}
		closex();
	}
