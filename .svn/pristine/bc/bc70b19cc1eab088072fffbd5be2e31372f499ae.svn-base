    ...
    <div id="grid_org_list_filter">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"选择")%> onclick="saveMenuIcon()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"审批")%> onclick="approve()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"转办")%> onclick="turnto()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"加签")%> onclick="addto()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"流程图")%> onclick="actPicture()"/>
			</div>
...
$(function() {
    ...
    function querys_1041() {
        $("#org_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/act/task/todo.action',
            ...
            },
			{
                ...
                formatter: function (value, row, index) {
					...
					return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"title\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"任务标题\">"+value+"</a>";
				}
            },
            {
            ...
            },
			 {
                ...
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"taskName\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"任务名称\">"+value+"</a>";
                }
            },
            {
            ...
            },
			{
                ...
                formatter: function (value, row, index) {
					...
					return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"businessId\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"业务绑定ID\">"+value+"</a>";
				}
            },
            ...
            },
			{
                ...
                formatter: function (value, row, index) {
					...
					return "<a class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"actCreateTime\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"任务到达时间\">" + date + "</a>";
				}
            },
            ],
...
function approve(){
	...
	//var url='/myehr/jsp/act/actManage/formTest/completeAttLeave.jsp?isInit=true&taskId='+select.taskId+'&procInsId='+select.procInsId+'&businessId='+select.businessId;
...
function turnto(){
	...
	var url='/myehr/jsp/act/actManage/userList.jsp?procInsId='+select.procInsId+'&taskDefKey='+select.taskDefKey;
	
	parent.layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"流程转办")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function addto(){
	...
	var url ='${pageContext.request.contextPath }/act/task/addMoreTask.action?procDefId='+select.procDefId+'&procInsId='+select.procInsId+'&taskDefKey='+select.taskDefKey;
...
function actPicture(){
	...
	var url='/myehr/act/task/trace/photo.action?procInsId='+procInsId;
...
function grid_org_list_fun(pageReqeust){
	...
	var paramsMap = {userId:userId,showType:'all'};
	var requestParam={};
