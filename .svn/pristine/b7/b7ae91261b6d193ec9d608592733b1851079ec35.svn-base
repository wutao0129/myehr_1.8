    ...
    <div id="grid_org_list_filter">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"下线")%> onclick="selectUsers()"/>
			</div>
		</div>
...
$(function() {
    ...
    function querys() {
        $("#scheme_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/SysUserList/queryUserList.action',
            ...
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"登录时间")%>',
                field : 'userLoginTime',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    	...
                    	return "<a class=\"eli w200\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"userLoginTime\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"登录时间\">" + date + "</a>";
                	}
            },
            ],
...
function selectUsers(){
	...
	if(selects.length>0){
		for(var i=0;i<selects.length;i++){
			userIds+=selects[i].userId+',';
		}
	}else{
		alert('请选择用户');
	}
	$.ajax({
		url: '/myehr/SysUserList/downlineUsers.action?userIds='+userIds,
		type:'post',
		cache: false,
		contentType: 'application/json;charset=utf-8',
