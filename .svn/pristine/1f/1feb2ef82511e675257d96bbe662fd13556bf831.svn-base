...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div role="tabpanel" class="tab-pane panel panel-primary fade in active" id="Section1" style="width:27%;float:left;height:93%;margin:0">
		<div class="panel-heading">题库模板树</div>
		<!--
		<div style="margin-right:10px;display:inline-block">
		    <input type="button" id="checkMenus" style="margin-left:10px;margin-top:5px" class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"复制权限")%> onclick="checkRoles()"/>
		</div>
		-->
	...
	<div id="iframe" title="center" region="center" style="border:0;float:left;width:62%;height:100%;">
	<iframe id="main" frameborder="0" name="main"  width="60%" height="100%" border="0" scroll="yes" style="margin-top: 0px;" ></iframe>
	</div>
</div>
...
var setting = {
			...
			check: {enable:false,chkboxType: {"Y":"s", "N":"ps"}},
			data: {
				key: {title: "title"},
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/SysRoleList/questionTemplateTree.action?formId=${param.formId}",//ajax的请求地址
		...
		var paramtabs_map = {Template: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"模板下检查点")%>', url:'<%= basePath%>jsp/createJsp/checkedList.jsp'}]
							 ,Question:[{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"检查点下指标")%>', url:'<%= basePath%>jsp/createJsp/checkedPointList.jsp'}]
							};
		...
		function onAsyncSuccess(event, treeId, treeNode){
				...
				iFrameWidth1 = iFrameWidth*0.82;
				iFrameWidth2 = iFrameWidth*0.17;
				
				$("#tabpanel").width(iFrameWidth2+"px");
				$("#main").width(iFrameWidth1+"px");
...
function setUrlParam(url,params){
	...
	url+="&isInit=true&roleId="+${param.roleId};
...
var ORG_ABBREVIATION = node.ORG_ABBREVIATION ? node.ORG_ABBREVIATION.toLowerCase() : "";
	...
	function checkRoles(){
		layer.open({ 
			...
			maxmin:true,
			zIndex:layer.zIndex,
			content: 'roleList.jsp?type=2&roleId='+${param.roleId},
