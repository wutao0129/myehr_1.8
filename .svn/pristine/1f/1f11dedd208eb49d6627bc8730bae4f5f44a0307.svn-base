...
public class SysFormButtonCalculateCache implements Serializable {
  ...
  //保留关联按钮对象
	private SysFormButtonCache button ;
	
	private List<SysSolutionCache> solutions = new ArrayList<SysSolutionCache>();
	...
	public SysFormButtonCache getButton() {
		return button;
	}

	public void setButton(SysFormButtonCache button) {
		this.button = button;
	}
	...
	public List<SysSolutionCache> getSolutions() {
		return solutions;
	}

	public void setSolutions(List<SysSolutionCache> solutions) {
		this.solutions = solutions;
	}
    
    public SysFormButtonCalculateCache(){};
    
    public SysFormButtonCalculateCache(SysFormButtonCalculate obj,SysFormButtonCache button){
    	//填充关联对象
		this.button = button;
		//填充对象
		this.pojo  = obj;
		...
		for( int i=0; i<objs.size(); i++){
			this.solutions.add(new SysSolutionCache(objs.get(i)));
		}
	...
	private void initParams(){
		for(SysSolutionCache solution:solutions){
			List<SysRuleCache> rules = solution.getRules();
			for(SysRuleCache rule:rules){
