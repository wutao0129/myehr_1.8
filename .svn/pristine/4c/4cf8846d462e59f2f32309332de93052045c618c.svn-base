    ...
    <div id="grid_org_list_filter">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"引入角色")%> onclick="introduceRoles()"/>
			</div>
		</div>
			<div id="grid_org_list_filter" style="float:right;display:inline-block;margin-right:155px;position:relative;">
				<input id="FILTER_1041" name="FILTER_1041" placeholder="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"角色编码")%>,<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"角色名称")%>" style="padding: 6px 30px 6px 10px;" title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"角色编码")%>,<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"角色名称")%>" type="text" class="form-control" oninput="grid_org_list_fun11()" />
...
$(function() {
    ...
    function querys() {
        $("#scheme_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/SysMenuList/querySchemeRoleList.action',
            ...
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"角色状态")%>',
                field : 'roleStatus',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    if(value==undefined){
                        value='-';
                    	  ...
                    	  return "<a  class=\"eli w50\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"roleStatus\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"角色状态\">"+value+"</a>";
                }
            },
            {
...
function introduceRoles(){
	...
	var url='/myehr/system/menu/roleList.jsp?schemeId='+schemeId;
