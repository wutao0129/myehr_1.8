...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div role="tabpanel" class="tab-pane panel panel-primary fade in active" id="Section1" style="width:17%;float:left;height:93%;margin:0">
		<div class="panel-heading">方案管理树</div>
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/scheme/buildSchemeTree.action?params={s_userId:userId}",//ajax的请求地址
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案列表")%>', url:'<%= basePath%>jsp/form/scheme/queryScheme.jsp'}],
							F: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式列表")%>', url:'<%= basePath%>jsp/form/scheme/queryFormula.jsp'}],
							T: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式编辑")%>', url:'<%= basePath%>jsp/form/scheme/editFormula.jsp'}]
							};		
...
var ORG_ABBREVIATION = node.ORG_ABBREVIATION ? node.ORG_ABBREVIATION.toLowerCase() : "";
	...
	function addNode(folderTreeParentId) {
		...
		layer.open({
			...
			yes: function(index, layero){
				...
				var url = "/myehr/scheme/addTreeSolutionFolder.action";//更新树节点信息
				$.ajax({
					url:url,
					type:'post',
					cache: false,
	...
	function deleteNode(folderTreeId,folderTreeName) {
		...
		layer.msg('确定要删除\"'+folderTreeName+'\"节点', {
			  ...
			  ,yes: function(index){
				...
				$.ajax({
			        url: "/myehr/scheme/deleteTreeSolutionFolder.action?folderTreeId="+folderTreeId,
			        type: 'POST',
			        cache: false,
			        async: false,
	...
	function setChildValue(formID,formName){
		window.frames["main"].document.getElementById("SYS_SYSTEM_SCHEME.ruleFormId").value=formID;
		window.frames["main"].document.getElementById("SYS_SYSTEM_SCHEME.ruleFormName").value=formName;
		window.frames["main"].buildFormField(formID);
		closex();
	}
	</script>
</html>
