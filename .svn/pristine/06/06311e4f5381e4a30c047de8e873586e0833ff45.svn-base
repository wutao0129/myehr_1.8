    ...
    <div id="grid_org_list_filter">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			...
			<div id="grid_org_list_filter" style="float:right;display:inline-block;margin-right:155px;position:relative;">
				<input id="FILTER_1041" name="FILTER_1041" placeholder="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式编码")%>,<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式名称")%>" style="padding: 6px 30px 6px 10px;" title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式编码")%>,<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式名称")%>" type="text" class="form-control" oninput="grid_org_list_fun11()" />
...
$(function() {
    ...
    function querys_1041() {
        $("#scheme_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/scheme/queryFormulaList.action',
            ...
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式编码")%>',
                field : 'ruleCode',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"ruleCode\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"公式编码\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式名称")%>',
                field : 'ruleName',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;color:black;width:150px;\" name=\"ruleName\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"公式名称\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"执行顺序")%>',
                field : 'ruleSort',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"ruleSort\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"执行顺序\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"默认执行")%>',
                field : 'ruleIsexc',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    if(value==1){
						return "<input   class=\"eli w100\" name=\"ruleIsexc\" type=\"checkbox\" value=\"1\" checked=\"checked\"\>";
                    }else{
						return "<input   class=\"eli w100\" name=\"ruleIsexc\" type=\"checkbox\" value=\"0\" \>";
					}
            ...
            },
			{
                ...
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w50\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"operatorTime\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"操作时间\">"+value+"</a>";
                }
            },
            {
...
function addScheme(){
	...
	var url='/myehr/jsp/form/scheme/editFormula.jsp?ruleSolutionId='+nodeId;
	
	layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"增加公式")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function editScheme(){
	...
	var url='/myehr/jsp/form/scheme/editFormula.jsp?ruleId='+select.ruleId;
	
	
	layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"修改公式")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function deleteScheme(){
	...
	$.ajax({
		url:'/myehr/scheme/deleteRule.action?ruleIds='+ruleIds,
		type:'post',
