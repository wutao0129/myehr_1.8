...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div role="tabpanel" class="tab-pane panel panel-primary fade in active" id="Section1" style="width:27%;float:left;height:93%;margin:0">
		<div class="panel-heading">动态表单树</div>
	...
	<div id="iframe" title="center" region="center" style="border:0;float:left;width:72%;height:100%;">
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/dictType/buildDictTree.action?roleId=${param.roleId}",//ajax的请求地址
		...
		var paramtabs_map = {F: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"表单字段按钮")%>', url:'<%= basePath%>jsp/dict/queryUserDictEntry.jsp'}]};
...
function setUrlParam(url,params){
	...
	url+="&isInit=true&userId=${param.userId}";
...
var ORG_ABBREVIATION = node.ORG_ABBREVIATION ? node.ORG_ABBREVIATION.toLowerCase() : "";
	...
	function checkRoles(){
		layer.open({ 
			type: 2,
			title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"复制权限")%>',
			shadeClose: true,
			shade: 0.8,
			area: ['500px','600px'],
			maxmin:true,
			zIndex:layer.zIndex,
			content: 'userDictCopyList.jsp?type=3&userId='+${param.userId},
