...
<div class="container-fluid">
	<div style="margin-top:5px;">
		<input id="SYS_ICON.buttonInterfaceId" name="buttonInterfaceId" type="hidden" class="form-control"  style="width:200px;"/>
		<input id="SYS_ICON.buttonInterfaceButtonId" name="buttonInterfaceButtonId" type="hidden" class="form-control"  style="width:200px;"/>
		...
		<h3 style=" background:#FFF;color:blue; display:block;font-size:12;position:absolute; left:10px; top:-27px;text-align:center"></h3>
			...
			<tr style="height:65px" id="tr1">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"接口方案")%>：</lable></td>
				<td style="padding-left:10px;"  align="left"  >	<input id="SYS_ICON.buttonInterfaceSchemeId" name="buttonInterfaceSchemeId" type="text" class="form-control"  style="width:200px;"/></td>
			</tr>
		</table>
	</div>
...
<script>
	...
	window.onload=function(){
		...
		$("#tr6").hide();
		initDict();
		document.getElementById("SYS_ICON.buttonInterfaceButtonId").value="${param.formButtonId}";
		var buttonInterfaceButtonId = '${param.formButtonId}';
		if(buttonInterfaceButtonId!='' && buttonInterfaceButtonId!=null){
			_initData(buttonInterfaceButtonId);
		}
	}
	...
	function save_formemp_emp_edit_click_1881(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/button/saveInterfaceButton.action',
			type:'post',
	...
	function _initData(pkId){
		var buttonInterfaceButtonId = pkId;
		
		var _form = getForm("#form_1881");
		var _Type = getType("#form_1881");
		$.ajax({
			url:'${pageContext.request.contextPath }/button/queryInterfaceButtonById.action?buttonInterfaceButtonId='+buttonInterfaceButtonId,
			type:'post',
			//data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
