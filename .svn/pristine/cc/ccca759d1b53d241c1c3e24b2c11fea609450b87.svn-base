(function ($) {
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $trs.each(function (key, item) { 
            $(this).find('th:gt(' + (that.options.fixedNumber - 1) + ')').remove();
            var $ths = $(this).find('th');
            var $allSeclect = $ths.find('input[name="btSelectAll"]');
            console.log($allSeclect);
            $allSeclect.click(function(){
                that.$header.find('td input[name="btSelectAll"]').click();
                that.$fixedBodyColumns.find('input').prop('checked',$(this).prop('checked'));  
    ...
    BootstrapTable.prototype.fitBodyColumns = function () {
        ...
        this.$fixedBody.find('tr[data-index]').on('click', function() {
            var index = $(this).data('index');
            _this.$body.find('tr[data-index="' + index + '"] ').click();
            console.log($(this).data('index'));
        });

        this.$fixedBody.find('tr[data-index] input').on('click', function() {
            var index = $(this).data('index');
            _this.$body.find('tr[data-index="' + index + '"] input').click();
            _this.$fixedBody.find('tr[data-index="' + index + '"]').toggleClass('selected');
            console.log($(this).data('index'));
        });
        ...
        this.$fixedBody.find('tr[data-index]').on('click', function() {
            var index = $(this).data('index');
            _this.$body.find('tr[data-index="' + index + '"] input').click();
            _this.$fixedBody.find('tr[data-index="' + index + '"] input').click();
            console.log($(this).data('index'));
        });
        this.$body.find('tr[data-index]').off('click').on('click', function() {
            var index = $(this).data('index');
            _this.$body.find('tr[data-index="' + index + '"] input').click();
            _this.$fixedBody.find('tr[data-index="' + index + '"] input').click();
        });
