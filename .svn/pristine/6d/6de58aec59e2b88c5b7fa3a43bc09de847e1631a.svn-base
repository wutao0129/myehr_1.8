...
<%
	String editorModelFolderId = request.getParameter("editorModelFolderId");
%>
...
<div class="container-fluid">
	<div style="margin-top:10px;">
		<div style="margin-top:10px;">
		<input id="SYS_SYSTEM_SCHEME.editorModelId" name="editorModelId" type="hidden" class="form-control"  style="width:200px;"/>
		<input id="SYS_SYSTEM_SCHEME.editorModelFolderId" name="editorModelFolderId" type="hidden" class="form-control"  style="width:200px;" value="<%=editorModelFolderId %>"/>
		<div style="border:1px solid #ccc; position:relative;border-radius:5px;margin-bottom:10px">
			...
			<table style="width:98%;table-layout:fixed;" class="cbs-table"  >
				<tr style="height:65px" width="100%">
					<td style="padding-left:10px;"  width="100px"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"模板编码")%>: </lable></td>
					<td style="padding-left:10px;"  align="left" width="200px"  ><input id="SYS_SYSTEM_SCHEME.editorModelCode" name="editorModelCode" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
					<td style="padding-left:10px;"  width="100px"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"模板名称")%>: </lable></td>
					<td style="padding-left:10px;"  align="left" width="200px"  ><input id="SYS_SYSTEM_SCHEME.editorModelName" name="editorModelName" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
					<td style="padding-left:10px;"  width="100px"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"选择表单")%>: </lable></td>
					<td style="padding-left:10px;"  align="left" width="200px"  >
						<input id="SYS_SYSTEM_SCHEME.editorModelFormName" name="editorModelFormName" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;position:absolute"/><i style="position:relative;left:180px;top:5px" onclick="choseForm()" class="glyphicon glyphicon-plus"></i>
						<input id="SYS_SYSTEM_SCHEME.editorModelFormid" name="editorModelFormid" type="hidden" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;position:absolute"/>
					</td>
				</tr>
			</table>
		</div>
...
var EDITOR_MODEL_VALUE = UM.getEditor('EDITOR_MODEL_VALUE',{autoHeightEnabled: false});
...
$(function () {  
	...
	function getdata_1881(buttonId,formId){
		...
		param.editorModelValue = txt;
		param.editorModelHtml = html;
	...
	window.onload=function(){
		...
		document.getElementById("SYS_SYSTEM_SCHEME.editorModelId").value="${param.editorModelId}";
		var editorModelId = '${param.editorModelId}';
		if(editorModelId!='' && editorModelId!=null){
			_initData(editorModelId);
		}
	...
	function save_formemp_emp_edit_click_1881(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/editorModel/saveEditorModel.action',
			type:'post',
	...
	function _initData(pkId){
		var editorModelId = pkId;
		var _form = getForm("#form_1881");
		var _Type = getType("#form_1881");
		$.ajax({
			url:'${pageContext.request.contextPath }/editorModel/queryEditorModel.action?editorModelId='+editorModelId,
			type:'post',
			//data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
			success: function (text) {
				if(text !=null && text!=""){
					var object = text;
					EDITOR_MODEL_VALUE.setContent(object.editorModelHtml);
					if(object.editorModelFormid){
						buildFormField(object.editorModelFormid);
					}
	...
	function choseForm(){
		var formId = 'SYS_SYSTEM_SCHEME.editorModelFormid';
		var formName = 'SYS_SYSTEM_SCHEME.editorModelFormName';
		var url = '/myehr/jsp/form/editorModel/formChoose.jsp?formId='+formId+'&formName='+formName;
		layer.open({
			type: 2,
