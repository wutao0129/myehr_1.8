...
<div title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"菜单树")%>" region="west" style="float:left;overflow:auto;width:17%;height:100%" bodyStyle="overflow:hidden;" showHeader="false"
	showFilter="false"
	...
	<input type="hidden" id="chooseValue">
	<div id="iframe" title="center" region="center" style="border:0;width:83%;height:100%;float:left">
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/dictType/buildDictTree.action?params={s_userId:userId}",//ajax的请求地址
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案列表")%>', url:'<%= basePath%>jsp/form/formcreate/queryDictTypeForBar.jsp'}],
							F: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式列表")%>', url:'<%= basePath%>jsp/form/formcreate/queryDictTypeForBar.jsp'}],
							F1: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公式列表")%>', url:'<%= basePath%>jsp/form/formcreate/queryDictTypeForBar.jsp'}],
							SQL:[{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"下拉SQL列表")%>', url:'<%= basePath%>jsp/form/formcreate/queryDictSqlListForBar.jsp'}]
							};		
...
var ORG_ABBREVIATION = node.ORG_ABBREVIATION ? node.ORG_ABBREVIATION.toLowerCase() : "";
	...
	function setValueForBar(){
		var id = '${param.id}';
		var result = $("#chooseValue").val();
		parent.document.getElementById(id).value = result;
		closex();
	}
