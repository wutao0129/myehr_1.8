<%@page pageEncoding="UTF-8" import="com.myehr.common.util.LangUtil"
...
#form-p-5,#form-p-4,#form-p-3,#form-p-2,#form-p-1 {
	width: 100%;
	height: 97%
}
...
.searchInfoWrapper>div {
	display: inline-block;
}

.searchInfoWrapper>div a {
	cursor: pointer;
}

.searchInfoWrapper label {
	margin: 4px 5px 0;
	vertical-align: top;
}
...
<body>
	<div id="layout1" class="nui-layout"
		...
		<div role="tabpanel"
			class="tab-pane panel panel-primary fade in active" id="Section1"
			style="width:17%;float:left;height:90%;margin:0;border:none">
			<div class="panel-heading"><%=LangUtil.getLangByCode(
					(String) session.getAttribute("langType"), "人员选择树")%></div>
			<div class="sidebar panel-body">
				<div class="searchInput">
					<input type="text" id="key" class="empty form-control"
						placeholder="Search..." onkeyup="callNumber()"> <i
						class="glyphicon glyphicon-search"
						style="position: absolute;right: 6px;top: 6px;cursor: pointer;"></i>
						<div class="searchInfo">
							<label type="text" class="searchInfoWrapper" id="resultKey">
								<div>
									<i id="clickUp" class="glyphicon glyphicon-arrow-up"
										onclick="clickUp()" style="top:4px"></i> <i id="clickDown"
										class="glyphicon glyphicon-arrow-down" onclick="clickDown()"
										style="top:4px"></i>
								</div> <label id="number"></label> </label>
						</div>
				</div>
				<div class="form-control" id="jsp"
					style="height:100%;padding:5px;overflow:auto;width:98%;margin:0 auto;">
					<div class="jsFiddle inline-buttons"">
						<textarea rows="5" style="width:200px" id="empContainer"
							cols="100"></textarea>
					</div>
					<ul id="tree_EMP_CHOSE_TREE"
						style="overflow-y: auto;overflow-x: hidden;height: 86%;margin-top: 10px;"
						class="ztree" showTreeIcon="true" checkRecursive="false"
						checkParent="false" expandOnLoad="0" showTreeLines="true"
						allowSelect="true" showCheckbox="false" showFolderCheckbox="false"
						expandOnDbclick="true" expandOnClick="true" textField="CONTENT"
						idField="DEPT_ID" resultAsTree="false" dataField="datas"
						onnodeclick="onNodeClick_EMP_CHOSE_TREE"
						onload="load_EMP_CHOSE_TREE" contextMenu="#treeMenu"
						value="ORG_DEL" parentField="PARENT_ID">
					</ul>
				</div>
			</div>
		</div>
		<div id="iframe" title="center" region="center"
			style="border:0;width:82%;height:100%;float:left;">
			<iframe id="main" frameborder="0" name="main" width="100%"
				height="100%" border="0" scroll="yes" style="margin-top: 0px;"></iframe>
		</div>
	</div>
</body>
...
var setting = {
			...
			data: {
				...
				simpleData: {
					enable: true,
					idKey: "DEPTCODE_DEPT_ID",
					pIdKey: "DEPTCODE_PARENT_ID",
					rootPId: 2 //根节点pid 
				}
			},
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/form/queryTreeSolution.action?formId=106&params={s_userId:'1'}",//ajax的请求地址
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode(
					(String) session.getAttribute("langType"), "人员选择树")%>', url:'<%=basePath%>form/toForm.action?formId=91'}]}		
//数据过滤  childNodes其实是ajax返回的数据
		function filter(treeId, parentNode, childNodes) {
				if (!childNodes) return null;
					for (var i=0, l=childNodes.length; i<l; i++) {
						childNodes[i].name = childNodes[i].DEPTCODE_CONTENT.replace(/\.n/g, '.');
					}
					return childNodes;
				}
		...
		function onAsyncSuccess(event, treeId, treeNode){
				...
				var zTree = $.fn.zTree.getZTreeObj("tree_EMP_CHOSE_TREE");
		...
		function onClick(event, treeId, treeNode){  
				...
				if(1>2){
					...
					var nodeType = treeNode.DEPTCODE_ORG_DEL;
		...
		function tree_EMP_CHOSE_TREE_fun(){
				var zTree = $.fn.zTree.getZTreeObj("tree_EMP_CHOSE_TREE");
				var nodes = zTree.getNodes();
				zTree.expandNode(nodes[0], true);
				var iFrameWidth = $(window).width()-width[0];
		...
		$("#jsp").css("height","100%");
		//zTree 初始化方法
			$.fn.zTree.init($("#tree_EMP_CHOSE_TREE"), setting);
...
function setUrlParam(url,params){
	...
	url+="&isInit=true";
	url+="";
	var empIds=document.getElementById("empContainer").value.split("\n");
	url+='&empIds='+empIds;
...
var CONTENT = node.CONTENT ? node.CONTENT.toLowerCase() : "";
	...
	function refreshNode() {
		if(!"true".equals("false")){
			var tree = nui.get("tree_EMP_CHOSE_TREE");
			tree.load("${pageContext.request.contextPath }/form/queryTreeSolution.action?formId=106&params={s_userId:'1'}");
		}
	}
	function closex_106() {    
