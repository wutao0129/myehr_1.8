...
public class ProcessInstanceHighlightsResource {
	private static Logger logger = LoggerFactory.getLogger(ProcessInstanceHighlightsResource.class);
	@Autowired
	...
	public ObjectNode getHighlighted(@PathVariable String processInstanceId) {
		...
		try {
			...
			List<HistoricActivityInstance> historicActivityInstances = historyService.createHistoricActivityInstanceQuery()
					...
					.list();
			if(processInstance==null&&historicActivityInstances!=null&&historicActivityInstances.size()>0){//流程结束高亮
				ProcessDefinitionId=historicActivityInstances.get(0).getProcessDefinitionId();
				highLightedActivities.add(historicActivityInstances.get(0).getActivityId());//当前节点
			}else{
	...
	private List<String> getHighLightedFlows(String processInstanceId, ProcessDefinitionEntity processDefinition) {
		...
		List<String> highLightedFlows = new ArrayList<String>();
		List<HistoricActivityInstance> historicActivityInstances = historyService.createHistoricActivityInstanceQuery()
				.processInstanceId(processInstanceId).orderByHistoricActivityInstanceStartTime().asc().orderByHistoricActivityInstanceEndTime().asc().list();
		...
		// add current activities to list
		 ...
		 List<HistoricActivityInstance> histor = historyService.createHistoricActivityInstanceQuery()
					.processInstanceId(processInstanceId).activityType("endEvent")
					.list();
		if(histor!=null&&histor.size()>0){
			//流程结束高亮 当前节点
			highLightedActivities.add(histor.get(0).getActivityId());
		}else{
			highLightedActivities = runtimeService.getActiveActivityIds(processInstanceId);
		}
		...
		for (ActivityImpl activity : processDefinition.getActivities()) {
			...
			List<Integer> allinde=	getindexofString(historicActivityInstanceList,activity.getId());
			if (index >= 0 && index + 1 < historicActivityInstanceList.size()) {
				List<PvmTransition> pvmTransitionList = activity.getOutgoingTransitions();
				for (PvmTransition pvmTransition : pvmTransitionList) {
					String destinationFlowId = pvmTransition.getDestination().getId();
					if(allinde!=null&&allinde.size()>0){
						for (int i = 0; i < allinde.size(); i++) {
							if (allinde.get(i)+1<historicActivityInstanceList.size()&&destinationFlowId.equals(historicActivityInstanceList.get(allinde.get(i) + 1))) {
	...
	public  List<Integer>  getindexofString(List<String> list,String aa){
		
		List<Integer> index=new ArrayList<Integer>();
		List<String> list1=new ArrayList<String>(list);
		for (int i = 0; i < list1.size(); i++) {
			if(list1.get(i).equals(aa)){
				index.add(i);
				list1.set(i, "1");
			}
		}
