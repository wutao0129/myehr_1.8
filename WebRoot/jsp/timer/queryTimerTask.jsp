	...
	<div style="margin-top:5px;">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="editorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"增加")%> onclick="addTimer()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="editorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"修改")%> onclick="editTimer()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="editorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"启动")%> onclick="startTimer()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="editorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"终止")%> onclick="stopTimer('stop')"/>
			</div>
		</div>
...
$(function() {
    ...
    function querys_1041() {
        $("#scheme_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/sysTask/querySystemTimer.action',
            ...
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"定时器Id")%>',
                field : 'sys_timer_id',
                align : 'center',
                visible : false,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"sys_timer_id\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"定时器Id\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"定时器名称")%>',
                field : 'sys_timer_name',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
					...
					return "<a  class=\"eli w100\" style=\"font-size:14px;color:black;width:200px;\" name=\"sys_timer_name\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"定时器名称\">"+value+"</a>";
            ...
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"定时器类型")%>',
                field : 'sys_timer_type',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    if(value==undefined){
                     ...
                     return "<a class=\"eli w200\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"sys_timer_type\" data-type=\"select\" data-pk=\""+row.Id+"\" data-value=\""+value+"\" data-title=\"定时器类型\" data-source=\"${pageContext.request.contextPath }/dict/searchSysDictEntryTypeCode1.action?deleteMark=TIMER_START_TYPE\"></a>";
                }
            },
			{
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"定时器状态")%>',
                field : 'sys_timer_status',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    if(value==undefined){
                     ...
                     return "<a class=\"eli w200\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"sys_timer_status\" data-type=\"select\" data-pk=\""+row.Id+"\" data-value=\""+value+"\" data-title=\"定时器类型\" data-source=\"${pageContext.request.contextPath }/dict/searchSysDictEntryTypeCode1.action?deleteMark=TIMER_STATUS\"></a>";
                }
            },
			],
...
function addTimer(){
	var url='/myehr/jsp/timer/editTimerTask.jsp';
	
	layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"增加定时任务")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function editTimer(){
	...
	var url='/myehr/jsp/timer/editTimerTask.jsp?sysTimerId='+select.sys_timer_id;
	
	layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"修改定时任务")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function startTimer(){
	...
	$.ajax({
		url:'${pageContext.request.contextPath }/sysTask/execSystemTimer.action?sysTimerId='+select.sys_timer_id,
		type:'post',
...
function stopTimer(timerType){
	...
	$.ajax({
		url:'${pageContext.request.contextPath }/sysTask/stopTimerById.action?sysTimerId='+select.sys_timer_id+'&timerType='+timerType,
		type:'post',
		//data: JSON.stringify(_param),
		cache: false,
		contentType: 'application/json;charset=utf-8',
