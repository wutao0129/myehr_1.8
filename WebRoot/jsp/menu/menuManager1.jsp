...
<div title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"菜单树")%>" region="west" style="float:left;width:100%;height:100%" bodyStyle="overflow:hidden;" showHeader="false" showFilter="false" class="sub-sidebar" allowResize="false">
	<div class="form-control" id="jsp" style="height:100%;padding:5px;">
		<div style="margin-right:10px;display:inline-block">
		    <input type="button" id="checkMenus"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"选择")%> onclick="checkMenus()"/>
		</div>
		<div style="margin-right:10px;display:inline-block">
		    <input type="button" id="checkMenus"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"复制权限")%> onclick="checkRoles()"/>
		</div>
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/SysMenuList/buildRoleMenuTree.action?roleId=${param.roleId}&timestamp="+timestamp,//ajax的请求地址
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"机构树")%>', url:'<%= basePath%>system/menu/menuList.jsp'}]};
...
var ORG_ABBREVIATION = node.ORG_ABBREVIATION ? node.ORG_ABBREVIATION.toLowerCase() : "";
	...
	function checkMenus(){
		...
		var zTree = $.fn.zTree.getZTreeObj("tree_org_tree");
		var checkMenus = zTree.getCheckedNodes(true);
		var _param={};
    	_param.roleId=roleId;
        _param.checkMenus=checkMenus;
		$.ajax({
            url: '/myehr/SysRoleList/saveMenuWithRole.action',
		    type:'post',
	...
	function checkRoles(){
		layer.open({ 
			type: 2,
			title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"角色列表")%>',
			shadeClose: true,
			shade: 0.8,
			area: ['500px','550px'],
			maxmin:true,
			zIndex:layer.zIndex,
			content: 'roleList.jsp?type=1&roleId='+${param.roleId},
