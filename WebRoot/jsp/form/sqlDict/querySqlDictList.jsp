...
$(function() {
    ...
    function querys_1041() {
        $("#scheme_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/FieldList/findDatafield.action',
            ...
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"主键Id")%>',
                field : 'dtId',
                align : 'center',
                visible : false,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"dtId\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"主键Id\">"+value+"</a>";
                }
            },
            {
            ...
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"SQL执行语句")%>',
                field : 'execSql',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
					...
					return "<div style=\"position:relative\"><input class=\"form-control\"  name=\"execSql\" data-type=\"text\" data-pk=\""+row.dtId+"\" data-title=\"SQL执行语句\" value=\""+value+"\"/>"+
							"<i class=\"glyphicon glyphicon-plus\" style=\"position: absolute;opacity: 0.5;right: 5px;top: 5px;cursor: pointer;\" onclick=\"editESQL("+row.dtId+")\"></i></div>";
				}
            },
            ],
...
function editESQL(daId){
	var datafieldId = daId;
	var execSqlType ='datafield_exec_sql';
	var title='设置【标准字段SQL下拉脚本】';
	
	url='/myehr/jsp/form/sqlDict/execute_datafield_sql.jsp?datafieldId='+datafieldId+'&execSqlType='+execSqlType;
