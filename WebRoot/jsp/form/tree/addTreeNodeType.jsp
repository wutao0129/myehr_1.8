...
<div class="container-fluid">
	<div style="margin-top:5px;">
		<input id="SYS_ICON.treeNodeTypeId" name="treeNodeTypeId" type="hidden" class="form-control"  style="width:200px;"/>
		...
		<table style="width:100%;table-layout:fixed;" class="cbs-table"  >
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"节点类型编码")%>：</lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_ICON.treeNodeTypeCode" name="treeNodeTypeCode" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"节点类型图标")%>：</lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_ICON.treeNodeTypeIcon" name="treeNodeTypeIcon" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
			</tr>
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"标题")%>：</lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_ICON.treeNodeTypeTittle" name="treeNodeTypeTittle" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"操作方案")%>：</lable></td>
				<td style="padding-left:10px;"  align="left"  >						<select id="SYS_ICON.treeNodeTypeExcType" onchange="changeeSlect2(this)"  title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"操作方案")%>" name="treeNodeTypeExcType"  class="form-control" emptyText="请选择..."  required="true"  style="width:200px;" textField="dictName" valueField="dictID" DictName="SYS_TREE_NODE_EXC_TYPE" nullItemText="" dataField="dicts"></select></td>
			</tr>
			<tr style="height:65px" id="tr_url">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"Url")%>：</lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_ICON.treeNodeTypeUrl" name="treeNodeTypeUrl" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
			</tr>
			<tr style="height:65px" id="tr_form">
				...
				<td style="padding-left:10px;"  align="left" >
					<input id="SYS_ICON.treeNodeTypeFormName" name="treeNodeTypeFormName" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;position:absolute"/><i style="position:relative;left:180px;top:5px" onclick="choseForm()" class="glyphicon glyphicon-plus"></i>
					<input id="SYS_ICON.treeNodeTypeFormId" name="treeNodeTypeFormId" type="hidden" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;position:absolute"/>
				</td>
			</tr>
		</table>
	</div>
...
<script>
	...
	window.onload=function(){
		$("#tr_url").hide();
		initDict();
		document.getElementById("SYS_ICON.treeNodeTypeId").value="${param.treeNodeTypeId}";
		document.getElementById("SYS_ICON.formTreeFormId").value="${param.formDefId}";
		var treeNodeTypeId = '${param.treeNodeTypeId}';
		if(treeNodeTypeId!='' && treeNodeTypeId!=null){
			_initData(treeNodeTypeId);
		}
	}
	...
	function save_formemp_emp_edit_click_1881(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/tree/saveTreeNodeType.action',
			type:'post',
	...
	function _initData(pkId){
		var treeNodeTypeId = pkId;
		
		var _form = getForm("#form_1881");
		var _Type = getType("#form_1881");
		$.ajax({
			url:'${pageContext.request.contextPath }/tree/getTreeNodeTypeById.action?treeNodeTypeId='+treeNodeTypeId,
			type:'post',
			//data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
	...
	function choseForm(){
		var url = '/myehr/jsp/form/tree/formChoose.jsp';
		layer.open({
			type: 2,
	...
	function changeeSlect1(obj){
		if(obj.treeNodeTypeExcType=='url'){
			$("#tr_form").hide();
			$("#tr_url").show();
		}else{
			$("#tr_form").show();
