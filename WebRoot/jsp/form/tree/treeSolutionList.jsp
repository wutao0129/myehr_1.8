...
<div class="container-fluid" style="height:100%;overflow-y:auto;">
	<div style="margin-top:5px;">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			...
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="editorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"缓存数据")%> onclick="dataCache()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
		...
		<div id="grid_org_list_filter" style="float:right;display:inline-block;margin-right:155px;position:relative;">
			<input id="FILTER_1041" name="FILTER_1041" placeholder="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案编码")%>,<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案名称")%>" style="padding: 6px 30px 6px 10px;" title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案编码")%>,<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案名称")%>" type="text" class="form-control" oninput="grid_org_list_fun11()" />
...
$(function() {
    ...
    function querys_1041() {
        $("#scheme_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/tree/queryTreeSchemeList.action',
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"treeSolutionId\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"方案Id\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案编码")%>',
                field : 'treeSolutionCode',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"treeSolutionCode\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"方案编码\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案名称")%>',
                field : 'treeSolutionName',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;color:black;width:150px;\" name=\"treeSolutionName\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"方案名称\">"+value+"</a>";
                }
            },
			{
                ...
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w200\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"userName\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"操作者\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"操作者")%>',
                ...
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w200\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"operatorName\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"操作者\">"+value+"</a>";
                }
            },
			{
			...
			{
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"删除标志")%>',
                field : 'deleteMark',
                align : 'center',
                visible : false,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w200\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"deleteMark\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"删除标志\">"+value+"</a>";
                }
            },
            ],
...
function addScheme(){
	...
	var url='/myehr/jsp/form/tree/editTreeSolution.jsp?treeSolutionFolderId='+nodeId;
...
function editScheme(){
	...
	var url='/myehr/jsp/form/tree/editTreeSolution.jsp?treeSolutionId='+select.treeSolutionId;
	
	parent.layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"修改树方案")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function deleteScheme(){
	...
	for (var i=0;i<selects.length;i++){
		treeSolutionIds+=selects[i].treeSolutionId+',';
	}
	$.ajax({
		url:'${pageContext.request.contextPath }/tree/deleteTreeSolution.action?treeSolutionIds='+treeSolutionIds,
		type:'post',
		cache: false,
		contentType: 'application/json;charset=utf-8',
...
function dataCache(){
	...
	$.ajax({
		url:'${pageContext.request.contextPath }/tree/cacheTreeSolution.action?treeSolutionId='+selects[0].treeSolutionId,
		type:'post',
		cache: false,
		contentType: 'application/json;charset=utf-8',
		success: function (text) {
