...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div role="tabpanel" class="tab-pane panel panel-primary fade in active" id="Section1" style="width:17%;float:left;height:93%;margin:0">
		<div class="panel-heading">导航方案树</div>
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/tree/buildTreeSchemeFolder.action?params={s_userId:userId}",//ajax的请求地址
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"方案列表")%>', url:'<%= basePath%>jsp/form/tree/treeSolutionList.jsp'}],
	...
	function addNode(folderTreeParentId) {
		...
		layer.open({
			...
			yes: function(index, layero){
				...
				var url = "/myehr/tree/addTreeSchemeFolder.action";//更新树节点信息
				$.ajax({
					url:url,
					type:'post',
					cache: false,
	...
	function deleteNode(folderTreeId,folderTreeName) {
		layer.msg('确定要删除\"'+folderTreeName+'\"节点', {
			  ...
			  ,yes: function(index){
				...
				$.ajax({
			        url: "/myehr/tree/deleteTreeSchemeFolder.action?folderTreeId="+folderTreeId,
			        type: 'POST',
			        cache: false,
			        async: false,
