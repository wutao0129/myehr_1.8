...
function Query_Group(){
	$("#groupList").bootstrapTable({  
	    url : '/myehr/form/getGroupList.action?formId=${param.formDefId}', 
		height:'800',
	    queryParams :'',
	    undefinedText : '-',  
...
function remove(obj){
	...
	$.ajax({
		url:'/myehr/form/deleteFormGroup.action',
		type:'post',
		cache: false,
...
function saveData(){
	...
	$.ajax({
		url:'/myehr/form/saveFormGroup.action?formId=${param.formDefId}',
		type:'post',
		cache: false,
		contentType: 'application/json;charset=utf-8',
...
function getEntityData(){
	...
	$("#groupList>tbody").find("tr").each(function(){
		if($(this).find("select").eq(0).val()!=""){
			...
			table.groupFormDefId = '${param.formDefId}';
