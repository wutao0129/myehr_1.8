                                    ...
                                    <div style="margin-top:5px;">
                                        ...
                                        <h3 style="display:block;font-size:18;text-align:left;height: 36px;line-height: 33px;padding-left: 16px;color: #666;cursor: pointer;">基础信息</h3>
                                            <div class="row cell" id="form1" >
												...
												<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
													<div class="ui search selection dropdown entitybox field-control" style="display: inline-block;">
														<lable style="float:left">接口方案地址</lable>
														<input id="interfaceSchemeUrl" name="interfaceSchemeUrl" type="text" class="form-control" style="width:200px;" value=""/>
													</div>
                                                </div>
												<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
													<div class="ui search selection dropdown entitybox field-control" style="display: inline-block;">
														<lable style="float:left">接口方案用户</lable>
														<input id="interfaceSchemeUsercode" name="interfaceSchemeUsercode" type="text" class="form-control" style="width:200px;" value=""/>
													</div>
                                                </div>
												<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
													<div class="ui search selection dropdown entitybox field-control" style="display: inline-block;">
														<lable style="float:left">接口方案密码</lable>
														<input id="interfaceSchemePassword" name="interfaceSchemePassword" type="text" class="form-control" style="width:200px;" value=""/>
													</div>
                                                </div>
                                            </div>
...
function formInit(){
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#interfaceSchemeUrl").val(data.interfaceSchemeUrl);
			$("#interfaceSchemeUsercode").val(data.interfaceSchemeUsercode);
			$("#interfaceSchemePassword").val(data.interfaceSchemePassword);
...
function saveFormInfo(){
	...
	param.interfaceSchemeUrl = $("#interfaceSchemeUrl").val();				
	param.interfaceSchemeUsercode = $("#interfaceSchemeUsercode").val();				
	param.interfaceSchemePassword = $("#interfaceSchemePassword").val();					
	$.ajax({
...
function jsParamInit(){
	$("#jsList").bootstrapTable({  
        url : '${pageContext.request.contextPath }/form/findJsParamList.action?paramTypeValue='+'${param.interfaceSehemeId}', 
		height:'200',
        queryParams :'',
        undefinedText : '-',  
        ...
        {  
            ...
            formatter: function (value, row, index) {
            	...
            	return  "		<select id=\"formDefLayoutType\" title=\"表单布局类型\" styleType=\"select\" name=\"buttonParamFrom\" value=\""+value+"\"  class=\"form-control\" >\n"+
						...
						"			<option value=\"interface\" "+select5+">接口参数</option>\n"+								
						"		</select>\n";
        	}
        }, {  
            title : '参数值来源',  
...
function saveJs(){
	...
	for(var i=0;i<datas.length;i++){
		...
		//实体可修改部分是否修改(有一处不同,就需修改)
			...
			datas[i].paramType = 'INTERFACE|JS_VAR';
