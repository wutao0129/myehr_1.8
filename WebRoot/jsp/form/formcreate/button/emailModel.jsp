		...
		<div style="border:1px solid #ccc; position:relative;border-radius:5px;margin-bottom:10px">
			...
			<table style="width:98%;table-layout:fixed;" class="cbs-table"  >
				<tr style="height:65px" width="100%">
					<td style="padding-left:10px;"  width="100px"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"选择模板")%>: </lable></td>
					
					<td style="padding-left:10px;"  align="left" width="200px"  >
						<input id="SYS_SYSTEM_SCHEME.editorModelName" name="editorModelName" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;position:absolute"/><i style="position:relative;left:180px;top:5px" onclick="choseForm()" class="glyphicon glyphicon-plus"></i>
						<input id="SYS_SYSTEM_SCHEME.editorModelId" name="editorModelId" type="hidden" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;position:absolute"/>
					</td>
					<td style="padding-right:10px;"  width="100px"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"邮箱字段")%>: </lable></td>
					<td style="padding-left:10px;"  align="right" width="200px">
...
<link rel="stylesheet" href="${pageContext.request.contextPath }/common/js/richText/themes/default/css/umeditor.css" type="text/css"></link>
<script type="text/javascript" src="${pageContext.request.contextPath }/common/js/richText/third-party/template.min.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath }/common/js/richText/umeditor.config.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath }/common/js/richText/umeditor.min.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath }/common/js/richText/lang/zh-cn/zh-cn.js"></script>
<script>
var classes = $("select");
var parame={
	id:'',
...
function onClick(event, treeId, treeNode){  
	...
	if(treeId=='entityTreeDemo'){
		...
		EDITOR_MODEL_VALUE.execCommand('inserthtml','[c:'+treeNode.name+'|'+treeNode.code+'|'+treeNode.id+']');
	}
	EDITOR_MODEL_VALUE.focus();
}; 
...
$(function () {  
	...
	function save_formemp_emp_edit_click_1881(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/button/saveEmailModel.action',
			type:'post',
	...
	function initModel(modelId){
		$.ajax({
			url:'${pageContext.request.contextPath }/editorModel/queryEditorModel.action?editorModelId='+modelId,
			type:'post',
			//data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
	...
	function choseForm(){
		var modelId = 'SYS_SYSTEM_SCHEME.editorModelId';
		var modelName = 'SYS_SYSTEM_SCHEME.editorModelName';
		var url = '/myehr/jsp/form/editorModel/modelChooseManager.jsp?modelId='+modelId+'&modelName='+modelName;
		layer.open({
			type: 2,
			title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"选择模板")%>',
			shadeClose: true,
			shade: 0.8,
			maxmin:true,
			zIndex:9999,
