...
<div class="container-fluid">
	<div style="margin-top:5px;">
		<input id="SYS_ICON.buttonRemoveId" name="buttonRemoveId" type="hidden" class="form-control"  style="width:200px;"/>
		<input id="SYS_ICON.buttonRemoveButtonId" name="buttonRemoveButtonId" type="hidden" class="form-control"  style="width:200px;"/>
		...
		<h3 style=" background:#FFF;color:blue; display:block;font-size:12;position:absolute; left:10px; top:-27px;text-align:center"></h3>
			...
			<tr style="height:65px" id="tr1">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"删除方式")%>：</lable></td>
				<td style="padding-left:10px;"  align="left"  >						<select id="SYS_ICON.buttonRemoveType" onchange="changeeSlect1(this)"  title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"删除方式")%>" name="buttonRemoveType"  class="form-control" emptyText="请选择..."  required="true"  style="width:200px;" textField="dictName" valueField="dictID" DictName="SYS_FORM_BUTTON_REMOVE_TYPE" nullItemText="" dataField="dicts"></select></td>
				...
				<td style="padding-left:10px;"  align="left" class="td1" >						<select id="SYS_ICON.buttonRemoveExcType" onchange="changeeSlect2(this)" title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"操作方案")%>" name="buttonRemoveExcType"  class="form-control" emptyText="请选择..."  required="true"  style="width:200px;" textField="dictName" valueField="dictID" DictName="SYS_FORM_BUTTON_SAVE_EXC_TYPE" nullItemText="" dataField="dicts"></select></td>
			</tr>
			<tr id="tr5">
		        ...
		        <td colspan="3" align="left">
		        	<textarea  id="SYS_ICON.buttonRemoveInitFun" name="buttonRemoveInitFun"  class="form-control"  rows="5"  style="width:400;margin-bottom:5px"> </textarea>
		   		</td>
	      	</tr>
	      	<tr id="tr6">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"自定义操作方案")%>：</lable></td>
		        <td colspan="3" align="left">
		        	<textarea  id="SYS_ICON.buttonRemoveFun" name="buttonRemoveFun"  class="form-control"  rows="5"  style="width:400;margin-bottom:5px"> </textarea>
		   		</td>
	      	</tr>
		</table>
	</div>
...
<script>
	...
	window.onload=function(){
		...
		$("#tr6").hide();
		initDict();
		document.getElementById("SYS_ICON.buttonRemoveButtonId").value="${param.formButtonId}";
		var buttonRemoveButtonId = '${param.formButtonId}';
		if(buttonRemoveButtonId!='' && buttonRemoveButtonId!=null){
			_initData(buttonRemoveButtonId);
		}
	}
	...
	function save_formemp_emp_edit_click_1881(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/button/saveRemoveButton.action',
			type:'post',
	...
	function _initData(pkId){
		var buttonRemoveButtonId = pkId;
		
		var _form = getForm("#form_1881");
		var _Type = getType("#form_1881");
		$.ajax({
			url:'${pageContext.request.contextPath }/button/queryRemoveButtonById.action?buttonRemoveButtonId='+buttonRemoveButtonId,
			type:'post',
			//data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
			success: function (text) {
				if(text !=null && text!=""){
					...
					var obj={};
					obj.value = text.buttonRemoveType;
					changeeSlect1(obj);
					obj.value = text.buttonRemoveExcType;
					changeeSlect2(obj);
				}
			},
