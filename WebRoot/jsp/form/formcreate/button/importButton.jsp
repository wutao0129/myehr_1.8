...
<form id="form_1881" name="form_1881" >
	<div class=container-fluid style="position: relative;min-height:100%;">
		<div style="padding: 10px;">
			<input id="SYS_SYSTEM_SCHEME.buttonImportId" name="buttonImportId" type="hidden" class="form-control"  style="width:200px;"/>
					...
					<table style="width:100%;table-layout:fixed;" class="cbs-table"  >
						<tr style="height:40px" width="100%">
							...
							<td style="padding-left:10px;"  align="left" class="td1" >
								<select id="SYS_SYSTEM_SCHEME.buttonImportExcType" onchange="changeeSlect1(this)"  title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"按钮操作方案")%>" name="buttonImportExcType"  class="form-control" emptyText="请选择..."  required="true"  style="width:200px;" textField="dictName" valueField="dictID" DictName="SYS_FORM_BUTTON_SAVE_EXC_TYPE" nullItemText="请选择" dataField="dicts"></select>
							</td>
						</tr>
						<tr style="height:40px" width="100%">
							...
							<td colspan="3"  style="padding-left:10px;"  align="left">
								<input type="text" readonly="readonly" id="SYS_SYSTEM_SCHEME.buttonImportSql" name="buttonImportSql" placeholder="未选择任何文件" class="form-control"  style="width:200px;display:inline-block"/>
								<a id ="uoloadtemplate" href="#"></a>
								<input type="hidden" id="SYS_SYSTEM_SCHEME.buttonImportTemplate" name="buttonImportTemplate"/>
								<input type="hidden" id="SYS_SYSTEM_SCHEME.buttonImportUrl" name="buttonImportUrl"/>
						...
						<tr id="tr5">
							...
							<td colspan="3" style="padding-left:10px;"  align="left">
								<textarea  id="SYS_SYSTEM_SCHEME.buttonImportInitFun" name="buttonImportInitFun"  class="form-control"  rows="5"  style="width:400;margin-bottom:5px"> </textarea>
							</td>
						</tr>
						<tr id="tr6">
							<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"自定义操作方案")%>：</lable></td>
							<td colspan="3" style="padding-left:10px;"  align="left">
								<textarea  id="SYS_SYSTEM_SCHEME.buttonImportFun" name="buttonImportFun"  class="form-control"  rows="5"  style="width:400;margin-bottom:5px"> </textarea>
							</td>
						</tr>
					</table>
				</div>
			</div>
			<div style="border:1px solid #ccc;height:56%;position:relative;border-radius:5px;margin-bottom:10px;">
				<h3 style=" background:#FFF;color:blue; display:block;font-size:12;position:absolute; left:10px; top:-27px;text-align:center;z-index:999">设置字段翻译</h3>
...
$(function() {
    ...
    function querys_1041() {
        $("#scheme_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/button/queryImportColumnByButtonId.action',
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    return "<a style=\"font-size:14px;text-decoration:none;color:black;width:50px;\" name=\"defaultXH\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"序号\">"+value+"</a>"+
							"<input type=\"form-control\" name=\"IMPORT_COLUMN_ID\" style=\"width:0px;padding:0px;border:hidden\" readonly=true data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"主键Id\" value=\""+row.IMPORT_COLUMN_ID+"\"/>"+
			...
			{  
				...
				formatter: function (value, row, index) {
					...
					return "<select id=\"IMPORT_COLUMN_TYPE"+index+"\" class=\"form-control\"  name=\"IMPORT_COLUMN_TYPE\" DictName=\"SYS_COLUMN_IMPORT_TYPE\" dataField=\"\" data-type=\"text\" data-pk=\""+row.Id+"\" nullItemText=\"请选择\" data=\"\" data-title=\"翻译方式\" value=\""+value+"\"/>" ;
				}
            },
			{
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"关联字典")%>',
                field : 'IMPORT_COLUMN_DICT_TYPE',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"IMPORT_COLUMN_DICT_TYPE\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"关联字典\">"+value+"</a><i onclick=\"choseDict("+row.IMPORT_COLUMN_ID+")\" class=\"glyphicon glyphicon-plus\"></i>"+
					"<input type=\"form-control\" name=\"IMPORT_COLUMN_DICT_TYPE\" style=\"width:0px;padding:0px;border:hidden\" readonly=true data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"关联字典\" value=\""+value+"\"/>";
                }
            },
			{  
				title : '是否业务主键',  
				field : 'IMPORT_COLUMN_BUS_KEY',  
				align : 'center',  
				valign : 'middle', 
				width:200,
				formatter: function (value, row, index) {
					...
					return "<select id=\"IMPORT_COLUMN_BUS_KEY"+index+"\" class=\"form-control\"  name=\"IMPORT_COLUMN_BUS_KEY\" DictName=\"SYS_COMMON_YES_NO\" dataField=\"\" data=\"\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"是否业务主键\" value=\""+value+"\"/>" ;
				}
            },
			{  
				title : '禁止导入',  
				field : 'IMPORT_COLUMN_FORBID',  
				align : 'center',  
				valign : 'middle', 
				width:200,
				formatter: function (value, row, index) {
					...
					return "<select id=\"IMPORT_COLUMN_FORBID"+index+"\" class=\"form-control\"  name=\"IMPORT_COLUMN_FORBID\" DictName=\"IMPORT_FORBID\" dataField=\"\" data=\"\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"禁止导入\" value=\""+value+"\"/>" ;
				}
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"唯一约束")%>',
                field : 'IMPORT_COLUMN_UNIQUE',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<select id=\"IMPORT_COLUMN_UNIQUE"+index+"\" class=\"form-control\" name=\"IMPORT_COLUMN_UNIQUE\" DictName=\"\" dataField=\"sql_form_0000\" data-type=\"text\" data-pk=\""+row.Id+"\"  data-title=\"唯一约束\" value=\""+value+"\"/>" ;
                }
            },
			{
                ...
                formatter: function (value, row, index) {
                    ...
                    return "<input id=\"ESQL"+index+"\" name=\"ESQL\" type=\"hidden\"  class=\"eli w100\" style=\"font-size:14px;color:black;width:150px\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"关联SQL\"\>"+value+"<i onclick=\"choseForm("+row.IMPORT_COLUMN_ID+")\" class=\"glyphicon glyphicon-plus\"></i>";
                }
            },
            ],
...
$(function () {  
	...
	function save_formemp_emp_edit_click_1881(){
		...
		layer.load();
		$.ajax({
			url:'${pageContext.request.contextPath }/button/saveImportButton.action',
			type:'post',
			data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
	...
	function saveData(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/button/saveImportButtonColumn.action?formButtonId='+formButtonId,
			type:'post',
			cache: false,
			contentType: 'application/json;charset=utf-8',
	...
	function getEntityData(){
		...
		$("#scheme_list_List>tbody").find("tr").each(function(){
			var table = {};
			table.importColumnId=$(this).find("input").eq(1).val();
			table.importColumnRelId=$(this).find("input").eq(2).val();
			table.importColumnDictType=$(this).find("input").eq(3).val();
			table.importReColumnRelId=$(this).find("select").eq(0).val();
			table.importColumnType=$(this).find("select").eq(1).val();
			table.importColumnBusKey=$(this).find("select").eq(2).val();
			table.importColumnForbid=$(this).find("select").eq(3).val();
			table.importColumnUnique=$(this).find("select").eq(4).val();
	...
	function _initData(pkId){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/button/queryImportButtonById.action?formButtonId='+formButtonId,
			type:'post',
			//data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
	...
	function choseDict(importColumnId){
		var url = '${pageContext.request.contextPath }/jsp/form/formcreate/chooseDict.jsp?widget=importColumnDict&formId='+${param.formId}+'&importColumnId='+importColumnId;
		layer.open({
			type: 2,
			title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"编写脚本")%>',
			shadeClose: true,
