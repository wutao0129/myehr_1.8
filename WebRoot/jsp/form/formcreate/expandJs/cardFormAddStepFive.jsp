...
<head>
<style>
#columnList>tbody>tr>td,#tableListstep5>tbody>tr>td{padding:3px 6px;}
#columnList>tbody>tr>td input,#tableListstep5>tbody>tr>td input{border:0;}
#columnList>tbody>tr>td select,#tableListstep5>tbody>tr>td select{border:0;}
		...
		<div class="form-control" style="width:100%;height:100%;border:0">		
			<div class="" style="height:100%;overflow-y:hidden;padding:10px 0;">
				<div class="BTNGROUP_55" style="margin:0 auto; display:block;height: 35px;">
					...
					<div style="margin-right:10px;display:inline-block">
					    <input type="button" id="editButton"  class="btn btn-warning" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"编辑属性")%> onclick="editButton()"/>
					</div>
				</div>
    ...
    $("#tableListstep5").bootstrapTable({  
		...
		{  
			...
			formatter: function (value, row, index) {
            	...
            	return "<select id=\"formButtonType"+index+"\" name=\"formButtonType\" DictName=\"SYS_FORM_BUTTON_TYPE\" dataField=\"\" data-type=\"text\" data-pk=\""+row.Id+"\"  data-title=\"按钮类型名称\" value=\""+value+"\"/>" ;
        	}
		}, {  
            title : '按钮显示名称',  
...
function addRowstep5(){ 
    var strAppend = '<tr data-index="'+num+'">'+
		...
		'</tr>';
    num++;
    $("#tableListstep5 tbody").append(strAppend);       
}

function deletexxstep5(Index,obj){
	if(Index!=0){
...
var objx;
function chooseEntitystep5(obj){
	objx = obj;
	saveData();
	addRow();
}
...
function editButtonstep5(obj){
	var selects = $("#tableListstep5").bootstrapTable("getSelections");
	...
	if(formButtonType=="add"){
		url='/myehr/jsp/form/formcreate/button/addbutton_form.jsp?formButtonId='+formButtonId;
		title='设置【新增】按钮扩张属性';
	}else if(formButtonType=="edit"){
		url='/myehr/jsp/form/formcreate/button/updatebutton_form.jsp?formButtonId='+formButtonId;
		title='设置【修改】按钮扩张属性';
	}else if(formButtonType=="remove"){
		url='/myehr/jsp/form/formcreate/button/removebutton_form.jsp?formButtonId='+formButtonId;
		title='设置【删除】按钮扩张属性';
	}else if(formButtonType=="save"){
		url='/myehr/jsp/form/formcreate/button/savebutton_form.jsp?formButtonId='+formButtonId;
		title='设置【保存】按钮扩张属性';
