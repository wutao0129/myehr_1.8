...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"表单目录树")%>" region="west" style="float:left;overflow:auto;width:15%;height:100%"
		showFilter="false"
	...
	<div id="iframe" title="center" region="center" style="border:0;width:85%;height:100%;float:left">
...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div role="tabpanel" class="tab-pane panel panel-primary fade in active" id="Section1" style="width:17%;float:left;height:93%;margin:0">
		<div class="panel-heading">表单目录树</div>
...
var setting = {
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"表单目录树")%>', url:'<%= basePath%>jsp/form/formcreate/formtemplatebuild/templateList.jsp'}]}		
		...
		function onAsyncSuccess(event, treeId, treeNode){
				...
				var iFrameWidth = $(window).width()-width[0]-60;
		...
		function onClick(event, treeId, treeNode){  
				...
				var url = '<%= basePath%>jsp/form/formcreate/formtemplatebuild/templateList.jsp';
...
function OnRightClick(event, treeId, treeNode) {
		...
		"		<input id='addNode' name='addNode' type='button' class='btn btn-success' style='' onclick=\"addNode('"+treeNode.FOLDER_TREE_ID+"')\" value='新增节点'/>\n"+
		"	</div>\n"+
		"</div>\n"+
		"<div id='treeNodeOperate2' class='col-lg-4 col-md-4 col-sm-4 col-xs-4'>\n"+
		"	<div class='ui search selection dropdown entitybox field-control' style='display: inline-block;width:100%;'>\n"+
		"		<input id='editNode' name='editNode' type='button' class='btn btn-warning' style='' onclick=\"editNode('"+treeNode.FOLDER_TREE_ID+"','"+treeNode.FOLDER_TREE_NAME+"','"+treeNode.FOLDER_TREE_CODE+"','"+treeNode.FOLDER_TREE_DESC+"')\" value='修改节点'/>\n"+
		"	</div>\n"+
		"</div>\n"+
		"<div id='treeNodeOperate3' class='col-lg-4 col-md-4 col-sm-4 col-xs-4'>\n"+
		"	<div class='ui search selection dropdown entitybox field-control' style='display: inline-block;width:100%;'>\n"+
		"		<input id='deleteNode' name='deleteNode' type='button' class='btn btn-danger' style='' onclick=\"deleteNode('"+treeNode.FOLDER_TREE_ID+"','"+treeNode.FOLDER_TREE_NAME+"')\" value='删除节点'/>\n"+
		"	</div>\n"+
		"</div>\n"+
		"</div>\n";
		layer.open({
	...
	function addNode(folderTreeParentId) {
		...
		layer.open({
			...
			yes: function(index, layero){
				...
				var url = "/myehr/form/treeNodeOperation.action";//更新树节点信息
				$.ajax({
					url:url,
					type:'post',
					cache: false,
	...
	function deleteNode(folderTreeId,folderTreeName) {
		layer.msg('确定要删除\"'+folderTreeName+'\"节点', {
			  ...
			  ,yes: function(index){
				...
				$.ajax({
			        url: "/myehr/form/deleteTreeNode.action?folderTreeId="+folderTreeId,
			        type: 'POST',
			        cache: false,
			        async: false,
