...
$(function() {
    ...
    function querys_1041() {
        $("#scheme_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/editorModel/queryEditorModelList.action',
            ...
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"模板Id")%>',
                field : 'editorModelId',
                align : 'center',
                visible : false,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"editorModelId\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"模板Id\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"模板名称")%>',
                field : 'editorModelName',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"editorModelName\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"模板名称\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"模板编码")%>',
                field : 'editorModelCode',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;color:black;width:150px;\" name=\"editorModelCode\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"模板编码\">"+value+"</a>";
                }
            },
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"模板类型")%>',
                field : 'editorModelType',
                align : 'center',
                visible : true,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:60px;\" name=\"editorModelType\" data-type=\"select\" data-pk=\""+row.Id+"\" data-value=\""+value+"\" data-title=\"模板类型\" data-source=\"${pageContext.request.contextPath }/dict/searchSysDictEntryTypeCode1.action?deleteMark=SYS_SQL_PARAMS\"></a>";
                }
            },
            {
...
function addScheme(){
	...
	var url='/myehr/jsp/form/editorModel/editorModel.jsp?editorModelFolderId='+nodeId;
	
	parent.layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"增加模板")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function editScheme(){
	...
	var url='/myehr/jsp/form/editorModel/editorModel.jsp?editorModelId='+select.editorModelId;
...
function deleteScheme(){
	...
	if(selects!=null && selects.length>0){
		for(var i=0;i<selects.length;i++){
			selectId+=selects[i].editorModelId+",";
		}
	}else {
		alert('<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"请选择一行数据")%>');return ;
	}

	var url='${pageContext.request.contextPath }/editorModel/deleteEditorModel.action?editorModelId='+selectId;
	$.ajax({
			url:url,
			type:'post',
