...
var ProcessDiagramGenerator = {	
	...
	showFlowInfo: function(flow){
		...
		var TPL_FLOW_INFO = ''//'<div>{flow}</div>' 
				+ '<div><b>开始流程ID</b>: {sourceActivityId}</div>'
				+ '<div><b>结束流程ID</b>: {destinationActivityId}</div>'
//				+ '<div><b>是否为默认（主站）站点</b>: {isDefault}</div>'
...
//				+ '<div><b>isHighLighted</b>: {isHighLighted}</div>';
	...
	showActivityInfo: function(activity){
		...
		var values = {
			...
			content:activity.getProperty("content")
		};
		var TPL_ACTIVITY_INFO = '' 
				+ '<div><b>流程ID</b>: {activityId}</div>'
				+ '<div><b>流程名称</b>: {name}</div>'
				+ '<div><b>状态</b>: {type}</div>'
				+ '<div>{content}</div>';
