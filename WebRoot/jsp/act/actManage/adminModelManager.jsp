...
<div title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"菜单树")%>" region="west" style="float:left;overflow:auto;width:13%;height:100%" bodyStyle="overflow:hidden;" showHeader="false" showFilter="false" class="sub-sidebar" allowResize="false">
	...
	<div id="iframe" title="center" region="center" style="border:0;width:87%;float:left;height:100%;">
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/form/buildFlowTree.action?params={s_userId:userId}",//ajax的请求地址
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"流程管理树")%>', url:'<%= basePath%>jsp/act/actManage/modelList.jsp'}]};
		//数据过滤  childNodes其实是ajax返回的数据
		function filter(treeId, parentNode, childNodes) {
				if (!childNodes) return null;
					for (var i=0, l=childNodes.length; i<l; i++) {
						childNodes[i].name = childNodes[i].flowName.replace(/\.n/g, '.');
					}
					return childNodes;
				}
		...
		function onAsyncSuccess(event, treeId, treeNode){
				...
				var iFrameWidth = $(window).width()-width[0]-50;
