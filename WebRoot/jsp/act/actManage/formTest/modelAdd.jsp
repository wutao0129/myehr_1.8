	...
	<div style="margin-top:5px;">
		...
		<h3 style=" background:#FFF;color:blue; display:block;font-size:12;position:absolute; left:10px; top:-27px;text-align:center">流程信息</h3>
			...
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"流程类别")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  >
					<select id="processCategory" title="操作成功后处理" name="processCategory"  class="form-control" emptyText="请选择..."  required="true"  style="width:200px;" textField="dictName" valueField="dictID" DictName="act_flow_type" nullItemText="" dataField="dicts"></select>
				</td>
			</tr>
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"流程名称")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="processName" name="processName" type="text" class="form-control"  style="width:200px;"/></td>
			</tr>
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"流程Key")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="processKey" name="processKey" type="text" class="form-control"  style="width:200px;"/></td>
			</tr>
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"流程定义")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="processDescription" name="processDescription" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
			</tr>
		</table>
	</div>
...
$(function () {  
	...
	function save_formemp_emp_edit_click_1881(){
		...
		var category=document.getElementById("processCategory").value;
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/flowModel/createModel.action?category='+category+'&name='+name+'&key='+key+'&description='+description,
			type:'post',
			cache: false,
			contentType:'application/json;charset=utf-8',
			success: function (text) {
				if(text.createTime!=null){
