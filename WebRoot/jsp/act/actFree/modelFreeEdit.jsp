...
function init(){
  ...
  $.ajax({
				url : '/myehr/actfree/seeActfreeModelEdit.action?modelid='+modelid, 
				type:'post',
				cache: false,
				async:false,
			...
			//	alert(data.title);
				...
				if(data){
				$('[id="modelId"]').val(data.modelid);
				$('[id="modelKey"]').val(data.modelkey);
				$('[id="modelName"]').val(data.modelname);
				$('[id="SYS_ICON.buttonFormId"]').val(data.formid);
				$('[id="modelType"]').val(data.modeltype);
				$('[id="createName"]').val(data.createname);
				$('[id="createTime"]').val(formatDateTime(data.createtime));
				}else{
 ...
 function formatDateTime(inputTime) {    
	...
	function save_formemp_emp_edit_click_1881(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/actfree/updateActfreeModel.action',
			type:'post',
