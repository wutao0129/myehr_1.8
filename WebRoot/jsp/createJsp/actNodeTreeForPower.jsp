...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div role="tabpanel" class="tab-pane panel panel-primary fade in active" id="Section1" style="width:17%;float:left;height:93%;margin:0">
		<div class="panel-heading">流程检查点管理树</div>
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/question/getActDataForChecked.action?params={s_userId:userId}",//ajax的请求地址
		...
		var paramtabs_map = {Node: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"指标")%>', url:'<%= basePath%>jsp/createJsp/checkedGridForPower.jsp'}],
							 Model: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"检查点")%>', url:'<%= basePath%>jsp/createJsp/checkedGridForPower.jsp'}]}//4419		
		...
		function onAsyncSuccess(event, treeId, treeNode){
				...
				iFrameWidth = iFrameWidth*0.82;
				$("#main").width(iFrameWidth+"px");
						document.getElementById("main").src = setUrlParam(paramtabs_map["Node"][0].url+"?modelId="+nodes[0].children[0].nodeId, nodes[0].children[0]);
				}
		...
		function onClick(event, treeId, treeNode){  
				...
				if(1>2){
					...
					var url = '';
					if(nodeType=='Node'){
						url = paramtabs[0].url+'?modelId='+treeNode.pid;
					}else{
						url = paramtabs[0].url+'?modelId='+treeNode.nodeId;
					}
