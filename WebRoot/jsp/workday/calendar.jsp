	...
	<body id="mainContent" style="width:1600px;height:700px;">
        ...
        <div class="content" id="updateWork" style="background:#FFF;border-radius:3px;border:1px solid #ccc;height:100px;width:200px; position:absolute;right:700px;top:150px;display:none;z-index:998">
            <div>
	            ...
	            <input type="radio" name="rest" value="2" title="法定假日"/>法定假日
	            <input type="radio" name="rest" value="sc" title="删除"/>删除
            </div>
...
<script type="text/javascript">
	...
	var calendar = $('#calendar').fullCalendar({
		...
		events: function(start,end,callback) {
	        ...
	        FormatDate(end);
	        $.ajax({
	            url: '${pageContext.request.contextPath }/calendar/queryWorkDay.action?kguid='+kguid,
	            type: 'POST',
		        cache: false,
	...
	function save1(){
		...
		if (title.length>0) {
			...
			$.ajax({
	            url: '${pageContext.request.contextPath }/calendar/addWorkDay.action',
	...
	function init(){
		...
		$.ajax({
	            url: '${pageContext.request.contextPath }/calendar/InitRestCalendar.action',
	...
	function deleteThisMonth(){
		...
		$.ajax({
	            url: '${pageContext.request.contextPath }/calendar/ClearRestCalendar.action',
	            data: "kguid=" + kguid + "&itemDate=" + datetime,
		        cache: false,
		        contentType: 'application/json;charset=utf-8',
	            success: function(json) { // 获取当前月的数据
