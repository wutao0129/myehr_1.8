...
function initArrData(datas){
	var xAxisData=[];
	var seriesData=[];
	var xAxisColumn = 'menu_type';
	
	var url='${pageContext.request.contextPath }/charts/searchChartsType.action?xAxisColumn='+xAxisColumn;
	$.ajax({
		url:url,
		type:'post',
		cache: false,
		...
		success: function (text) {
			for(var i=0;i<text.length;i++){
				xAxisData.push(text[i].menu_type);
				var flag=0;
				for(var j=0;j<datas.length;j++){
					if(text[i].menu_type==datas[j].menuType){
						flag++;
					}
				}
...
function addMenu(){
	var url='/myehr/jsp/menu/addbutton_form.jsp?menuParentCode='+'${param.menuCode}'+'&menuSeq='+'${param.menuSeq}';
...
function editMenu(){
	...
	var url='/myehr/jsp/menu/addbutton_form.jsp?menuId='+select.menuId;
