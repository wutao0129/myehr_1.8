	...
	<div style="margin-top:5px;">
		...
		<h3 style=" background:#FFF;color:blue; display:block;font-size:12;position:absolute; left:10px; top:-27px;text-align:center"></h3>
			...
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"字典类编码")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_SYSTEM_SCHEME.dictTypeCode" name="dictTypeCode" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
				
			</tr>
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"字典类名称")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_SYSTEM_SCHEME.dictTypeName" name="dictTypeName" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
			</tr>
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"字典类分类")%>: </lable></td>
			...
			<tr style="height:65px">
				<td style="padding-left:10px;"  width="150"  ><lable><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"字典类描述")%>: </lable></td>
				<td style="padding-left:10px;"  align="left"  ><input id="SYS_SYSTEM_SCHEME.dictTypeRemark" name="dictTypeRemark" type="text" class="form-control"  vtype="illegalChar;maxLength:null;minLength:null;" style="width:200px;"/></td>
			</tr>
		</table>
	</div>
...
$(function () {  
	...
	window.onload=function(){
		...
		if(dictTypeId!='' && dictTypeId!=null){
			_initData(dictTypeId);
		}
	...
	function save_formemp_emp_edit_click_1881(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/dictType/saveDictType.action',
			type:'post',
			...
			success: function (text) {
				if(text[0]==0){
					...
					alert("<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"编码重复")%>");
				}else{
	...
	function _initData(pkId){
		...
		$.ajax({
			url:'${pageContext.request.contextPath }/dictType/queryDictTypeById.action?dictTypeId='+dictTypeId,
			type:'post',
			//data: JSON.stringify(_param),
			cache: false,
			contentType: 'application/json;charset=utf-8',
