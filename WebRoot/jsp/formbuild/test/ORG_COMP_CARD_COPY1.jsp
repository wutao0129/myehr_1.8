...
<div class="container-fluid" style="height:100%;overflow-y:hidden;padding-top:10px;">
	...
	<table id="ORG_COMP_CARD_COPY1_List">
	</table>
...
$(".input-group").css("padding-bottom","0px");
    ...
    initDict();
    initDate();
    console.log($(".BTNGROUP_2605>div").length);
    var DIVAMOUNT = 5;
    if($(".BTNGROUP_2605>div").length > DIVAMOUNT){
    	var len = $(".BTNGROUP_2605>div").length;
    	$(".BTNGROUP_2605>div:gt(4)").hide();
    	...
    	$(".BTNGROUP_2605").append(selectObj);
    	for(var i=DIVAMOUNT;i<len;i++){
    		var liObj = $('<li style="margin-left:14px;"></li>');
    		liObj.append($(".BTNGROUP_2605>div").eq(i).clone(true));
...
function initFilterParam(data){
	...
	var _formId_2605='2605';
    function querys_2605() {
        $("#ORG_COMP_CARD_COPY1_List").bootstrapTable({
            ...
            fixedColumns: false,
            fixedNumber:4,
            queryParams : grid_ORG_COMP_CARD_COPY1_fun,
            cache : false,
            pageSize : 10, 
            ...
            },
  {
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    return "<a class=\"eli wpx\" style=\"font-size:12px;text-decoration:none;color:black;width:200px;\" name=\"ORGCOMPANY_parentid\" data-type=\"select\" data-pk=\""+row.Id+"\" data-value=\""+value+"\" data-title=\"上级公司\" data-source=\"${pageContext.request.contextPath }/dict/searchSysDictEntryTypeCode1.action?deleteMark= data_form_38616&realValue="+value+"+\"></a>";
                }
            },
            ...
            },
  {
		...
		onLoadSuccess: function (aa, bb, cc) {
                $("#ORG_COMP_CARD_COPY1_List a").editable({
                    disabled: true,
                    emptytext: "-",
                    url: function (params) {
		...
		onDblClickCell: function (field, value,row,td) {
        	editORG_COMP_CARD_COPY1_click_2605('isView',row);  
         } ,
...
$(".BTNGROUP_2605").css("margin-top","-25px");
/** 刷新页面 */ 
function refresh_ORG_COMP_CARD_COPY1() { 
    $('#ORG_COMP_CARD_COPY1_List').bootstrapTable('refresh'); 
}
/**查询条件与分页数据 */
...
var filterParam = {};
var queryParams;
function grid_ORG_COMP_CARD_COPY1_fun(pageReqeust){
	var _filterdata={};
	if(document.getElementById('grid_ORG_COMP_CARD_COPY1_filter')){
	if($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2605']").attr('multiSelect')){
		var val = "";
		var arr = $("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2605']").val();
		if(arr==null){
		}else{
	...
	}else if(($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2605']").attr('styleType')=="select")||($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2605']").attr('styleType')=="dateTime")){
			var ORGCOMPANY_cname=$("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2605']").val();
		}else{
				var ORGCOMPANY_cname=$("[id='FILTER_2605']").val();
		}
	if($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2605']").attr('multiSelect')){
		var val = "";
		var arr = $("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2605']").val();
		if(arr==null){
		}else{
			for(var i=0;i<arr.length;i++){
				val+=arr[i]+',';
	...
	}else if(($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2605']").attr('styleType')=="select")||($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2605']").attr('styleType')=="dateTime")){
			var ORGCOMPANY_ename=$("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2605']").val();
		}else{
				var ORGCOMPANY_ename=$("[id='FILTER_2605']").val();
		}
		_filterdata={ORGCOMPANY_cname:ORGCOMPANY_cname,ORGCOMPANY_ename:ORGCOMPANY_ename};
	}
	var paramsMap = {};
	var requestParam={};
	if('${param.orgcode}'==''){
	...
	pageReqeust.formId=_formId_2605;
...
function grid_ORG_COMP_CARD_COPY1_fun11_2605(){
	$('#ORG_COMP_CARD_COPY1_List').bootstrapTable('refresh');
}
/**下拉控件 */
var classes = $("select");
...
function getQueryString(name) {
	...
	function grid_ORG_COMP_CARD_COPY1_HeighQuery_2605() {
		layer.open({
			type: 2,
			showMaxButton: true,
			...
			area: ['855', '500'],
			maxmin: true,
			content:'/myehr/jsp/form/button/heightGrade.jsp?formFunction=grid_ORG_COMP_CARD_COPY1_fun11_2605&formDefId=2605&formDefLayout=2',
