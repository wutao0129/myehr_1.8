...
<div class="container-fluid" style="height:100%;overflow-y:hidden;padding-top:10px;">
	<div class="input-group" style="padding-bottom:10px;width:100%;z-index: 5;">
	    <div id="grid_ORG_COMP_CARD_COPY3_filter">
	        <table class="cbs-table" style="width:100%">
	             <tr>
					...
					<td style="padding-left:10px;padding-top:10px" null align="left"  >						<select id="FILTER_ORG_COMPANY.ORGCOMPANY_comptype_2607" title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"公司类型")%>" styleType="select" name="ORGCOMPANY_comptype"  class="form-control " emptyText="请选择..."  style="" textField="dictName" valueField="dictID" DictName="COMPTYPE" dataField="dictList_form_38673"></select></td>
					<td style="padding-left:10px;padding-top:10px"  width="100"  ><lable style="float:left"><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"成立日期")%>: </lable></td>
					<td style="padding-left:10px;padding-top:10px" null align="left"  >
						<div class='controls input-append date form_date1 input-group' data-date=""  data-link-field="dtp_input2" data-link-format="yyyy-mm-dd" style="float:left;width: 134px;">
							<input type='text' readonly class="form-control " dateType="controls input-append date form_date1" id="_INTERVAIL_MIN_FILTER_ORG_COMPANY.ORGCOMPANY_createdate_2607" styleType="dateTime"   style="width:134px;float:left;" name="_INTERVAIL_MIN_ORGCOMPANY_createdate"  format="yyyy-MM-dd" valueformat="yyyy-MM-dd"  timeFormat="HH:mm:ss"  showClearButton="true" showTodayButton="true"/>
						...
						<div class='controls input-append date form_date1 input-group' data-date=""  data-link-field="dtp_input2" data-link-format="yyyy-mm-dd" style="float:left;width: 134px;">
							<input type='text' readonly class="form-control " dateType="controls input-append date form_date1" id="_INTERVAIL_MAX_FILTER_ORG_COMPANY.ORGCOMPANY_createdate_2607" styleType="dateTime"   style="width:134px;float:left;" name="_INTERVAIL_MAX_ORGCOMPANY_createdate"  format="yyyy-MM-dd" valueformat="yyyy-MM-dd"  timeFormat="HH:mm:ss"  showClearButton="true" showTodayButton="true"/>
								<span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
								<span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span>
						</div>
					</td>
					<td style="float:right;padding-top:10px">
						<input id="FILTER_2607" name="FILTER_2607" placeholder="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"中文名称")%>,<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"英文名称")%>" style="padding: 6px 30px 6px 10px;" title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"中文名称")%>,<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"英文名称")%>" type="text" class="form-control" oninput="grid_ORG_COMP_CARD_COPY3_fun11_2607()" />
	                ...
	                <td style="margin-left:10px;float:left;padding-top:10px" align="left" >
						<div style="float:left;display:inline-block;margin-right:15px;position:relative;">
							<input class="btn btn-primary" style="width:70px;padding-left:2px;padding-right:25px;" value="查询" onclick="grid_ORG_COMP_CARD_COPY3_fun11_2607()">
	...
	<div class="BTNGROUP BTNGROUP_2607" style="margin:5px auto; display:inline-block;height: 30px;z-index: 6;position: relative">
		<div style="margin-right:10px;display:inline-block">
		    <input type="button" id="removeORG_COMP_CARD_COPY3"  class="btn btn-danger" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"删除")%> onclick="removeORG_COMP_CARD_COPY3_click_2607()"/>
		</div>
	</div>
	<table id="ORG_COMP_CARD_COPY3_List">
	</table>
...
$(".input-group").css("z-index","7");
    ...
    initDict();
    initDate();
    console.log($(".BTNGROUP_2607>div").length);
    var DIVAMOUNT = 5;
    if($(".BTNGROUP_2607>div").length > DIVAMOUNT){
    	var len = $(".BTNGROUP_2607>div").length;
    	$(".BTNGROUP_2607>div:gt(4)").hide();
    	...
    	$(".BTNGROUP_2607").append(selectObj);
    	for(var i=DIVAMOUNT;i<len;i++){
    		var liObj = $('<li style="margin-left:14px;"></li>');
    		liObj.append($(".BTNGROUP_2607>div").eq(i).clone(true));
...
function initFilterParam(data){
	...
	var _formId_2607='2607';
    function querys_2607() {
        $("#ORG_COMP_CARD_COPY3_List").bootstrapTable({
            ...
            fixedColumns: false,
            fixedNumber:4,
            queryParams : grid_ORG_COMP_CARD_COPY3_fun,
            cache : false,
            pageSize : 10, 
            ...
            },
  {
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    return "<a class=\"eli wpx\" style=\"font-size:12px;text-decoration:none;color:black;width:200px;\" name=\"ORGCOMPANY_parentid\" data-type=\"select\" data-pk=\""+row.Id+"\" data-value=\""+value+"\" data-title=\"上级公司\" data-source=\"${pageContext.request.contextPath }/dict/searchSysDictEntryTypeCode1.action?deleteMark= data_form_38672&realValue="+value+"+\"></a>";
                }
            },
            ...
            },
  {
		...
		onLoadSuccess: function (aa, bb, cc) {
                $("#ORG_COMP_CARD_COPY3_List a").editable({
                    disabled: true,
                    emptytext: "-",
                    url: function (params) {
		...
		onDblClickCell: function (field, value,row,td) {
        	editORG_COMP_CARD_COPY3_click_2607('isView',row);  
         } ,
...
/** 刷新页面 */ 
function refresh_ORG_COMP_CARD_COPY3() { 
    $('#ORG_COMP_CARD_COPY3_List').bootstrapTable('refresh'); 
}
/**查询条件与分页数据 */
...
function removeORG_COMP_CARD_COPY3_click_2607(){
	var selects = $("#ORG_COMP_CARD_COPY3_List").bootstrapTable('getSelections');;
	...
	if (r==true){
		...
		var  _param ={formId:_formId_2607,params:_rowvalids};
...
var filterParam = {};
var queryParams;
function grid_ORG_COMP_CARD_COPY3_fun(pageReqeust){
	var _filterdata={};
	if(document.getElementById('grid_ORG_COMP_CARD_COPY3_filter')){
	if($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_comptype_2607']").attr('multiSelect')){
		var val = "";
		var arr = $("[id='FILTER_ORG_COMPANY.ORGCOMPANY_comptype_2607']").val();
		if(arr==null){
		}else{
	...
	}else if(($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_comptype_2607']").attr('styleType')=="select")||($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_comptype_2607']").attr('styleType')=="dateTime")){
			var ORGCOMPANY_comptype=$("[id='FILTER_ORG_COMPANY.ORGCOMPANY_comptype_2607']").val();
		}else{
				var ORGCOMPANY_comptype=$("[id='FILTER_2607']").val();
		}
		var _INTERVAIL_MIN_createdate=document.getElementById('_INTERVAIL_MIN_FILTER_ORG_COMPANY.ORGCOMPANY_createdate_2607').value;
		var _INTERVAIL_MAX_createdate=document.getElementById('_INTERVAIL_MAX_FILTER_ORG_COMPANY.ORGCOMPANY_createdate_2607').value;
	if($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2607']").attr('multiSelect')){
		var val = "";
		var arr = $("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2607']").val();
		if(arr==null){
		}else{
			for(var i=0;i<arr.length;i++){
				val+=arr[i]+',';
	...
	}else if(($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2607']").attr('styleType')=="select")||($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2607']").attr('styleType')=="dateTime")){
			var ORGCOMPANY_cname=$("[id='FILTER_ORG_COMPANY.ORGCOMPANY_cname_2607']").val();
		}else{
				var ORGCOMPANY_cname=$("[id='FILTER_2607']").val();
		}
	if($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2607']").attr('multiSelect')){
		var val = "";
		var arr = $("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2607']").val();
		if(arr==null){
		}else{
			for(var i=0;i<arr.length;i++){
				val+=arr[i]+',';
		...
		var ORGCOMPANY_ename=val;
	}else if(($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2607']").attr('styleType')=="select")||($("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2607']").attr('styleType')=="dateTime")){
			var ORGCOMPANY_ename=$("[id='FILTER_ORG_COMPANY.ORGCOMPANY_ename_2607']").val();
		}else{
				var ORGCOMPANY_ename=$("[id='FILTER_2607']").val();
		}
		_filterdata={ORGCOMPANY_comptype:ORGCOMPANY_comptype,_INTERVAIL_MIN_ORGCOMPANY_createdate:_INTERVAIL_MIN_createdate,_INTERVAIL_MAX_ORGCOMPANY_createdate:_INTERVAIL_MAX_createdate,ORGCOMPANY_cname:ORGCOMPANY_cname,ORGCOMPANY_ename:ORGCOMPANY_ename};
	}
	var paramsMap = {};
	var requestParam={};
	if('${param.orgcode}'==''){
	...
	pageReqeust.formId=_formId_2607;
...
function grid_ORG_COMP_CARD_COPY3_fun11_2607(){
	$('#ORG_COMP_CARD_COPY3_List').bootstrapTable('refresh');
}
/**下拉控件 */
var classes = $("select");
...
function getQueryString(name) {
	...
	function grid_ORG_COMP_CARD_COPY3_HeighQuery_2607() {
		layer.open({
			type: 2,
			showMaxButton: true,
			...
			area: ['855', '500'],
			maxmin: true,
			content:'/myehr/jsp/form/button/heightGrade.jsp?formFunction=grid_ORG_COMP_CARD_COPY3_fun11_2607&formDefId=2607&formDefLayout=2',
