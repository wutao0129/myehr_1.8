...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;position:relative">
	<div role="tabpanel" class="tab-pane panel panel-primary fade in active" id="Section1" style="width:17%;float:left;height:90%;margin:0;border:none">
			<div class="panel-heading"><%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"岗位树")%></div>
				<div class="sidebar panel-body">
					...
					<ul id="tree_post_info_tree" style="overflow-y: auto;overflow-x: auto;height: 86%;margin-top: 10px;" class="ztree" 
			...
			expandOnClick="true"
			textField="CNAME" idField="ORGVORGANIZATIONALLA_ID" resultAsTree="false" dataField="datas"
			onnodeclick="onNodeClick_post_info_tree" onload="load_post_info_tree" contextMenu="#treeMenu"value="isdisabled"parentField="PARENTID" >
		</ul>
	</div>
	</div>
...
var setting = {
			...
			data: {
				...
				simpleData: {
					enable: true,
					idKey: "ORGVORGANIZATIONALLA_ID",
					pIdKey: "ORGVORGANIZATIONALLA_PARENTID",
					rootPId: 2 //根节点pid 
				}
			},
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/form/queryTreeSolution.action?formId=4572&params={}",//ajax的请求地址
				autoParam:["ORGVORGANIZATIONALLA_ID","ORGVORGANIZATIONALLA_XTYPE"], //传递的参数 id lv（层级，最开始是0）
				dataFilter: filter
			},
			//双击节点时，是否自动展开父节点的标识
			view: { dblClickExpand: false,showLine: false,addDiyDom: addDiyDom}, 
		};
		var paramtabs_map = {0: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"岗位信息")%>', url:'<%= basePath%>form/toForm.action?formId=2084'}]}		
//数据过滤  childNodes其实是ajax返回的数据
		function filter(treeId, parentNode, childNodes) {
				if (!childNodes) return null;
					for (var i=0, l=childNodes.length; i<l; i++) {
						if (childNodes[i].ORGVORGANIZATIONALLA_CNAME!=null) {
							childNodes[i].name = childNodes[i].ORGVORGANIZATIONALLA_CNAME.replace(/\.n/g, '.');
						}
					}
					return childNodes;
				}
		...
		function onAsyncSuccess(event, treeId, treeNode){
				...
				var zTree = $.fn.zTree.getZTreeObj("tree_post_info_tree");
		...
		function onClick(event, treeId, treeNode){  
				...
				if(1>2){
					...
					var nodeType = treeNode.ORGVORGANIZATIONALLA_isdisabled;
		...
		function tree_post_info_tree_fun(){
				var zTree = $.fn.zTree.getZTreeObj("tree_post_info_tree");
				var nodes = zTree.getNodes();
				zTree.expandNode(nodes[0], true);
				var iFrameWidth = $(window).width()-width[0];
		...
		$("#jsp").css("height","100%");
		//zTree 初始化方法
			$.fn.zTree.init($("#tree_post_info_tree"), setting);
...
function setUrlParam(url,params){
	...
	if(url.indexOf("?")>=0){
		url =  url + "&aa=dd&intrToId=${param.intrToId}&intrButtonId=${param.intrButtonId}"+treeUrl;
	}else{
		url =  url + "?aa=dd";
	}
	if(!url){
	...
	url+="&isInit=true&textField="+$("#tree_post_info_tree").attr("textField")+"&idField="+$("#tree_post_info_tree").attr("idField");
	url+="";
...
var CNAME = node.CNAME ? node.CNAME.toLowerCase() : "";
	...
	function refreshNode() {
		if(!"true".equals("false")){
			var tree = nui.get("tree_post_info_tree");
			tree.load("${pageContext.request.contextPath }/form/queryTreeSolution.action?formId=2499&params={}");
		}
	}
	function closex_2499() {    
