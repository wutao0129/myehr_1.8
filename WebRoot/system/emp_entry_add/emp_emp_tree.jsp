...
<div title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"人员机构树")%>" region="west" style="float:left;overflow:auto;width:220px;height:100%" bodyStyle="overflow:hidden;" showHeader="false"
	showFilter="false"
	class="sub-sidebar" allowResize="false">
	<div class="form-control" id="jsp" style="height:100%;padding:5px;">
		<ul id="tree_emp_emp_tree" class="ztree" 
value="ORG_DEL"parentField="ORG_PARENT_ID" >
		</ul>
	</div>
	</div>
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/form/queryTreeSolution.action?formId=1209&params={s_userId:'1'}",//ajax的请求地址
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"人员机构树")%>', url:'<%= basePath%>form/toForm.action?formId=1208'}]}		
//数据过滤  childNodes其实是ajax返回的数据
		...
		function onAsyncSuccess(event, treeId, treeNode){
				...
				var zTree = $.fn.zTree.getZTreeObj("tree_emp_emp_tree");
		...
		$("#jsp").css("height","100%");
		//zTree 初始化方法
			$.fn.zTree.init($("#tree_emp_emp_tree"), setting);
...
var ORG_ABBREVIATION = node.ORG_ABBREVIATION ? node.ORG_ABBREVIATION.toLowerCase() : "";
	...
	function GetData(){
		if(_treegridselectedrows!=null){
			...
			var selectTree = nui.get('tree_emp_emp_tree');  
	...
	function refreshNode() {
		if(!"true".equals("false")){
			var tree = nui.get("tree_emp_emp_tree");
			tree.load("${pageContext.request.contextPath }/form/queryTreeSolution.action?formId=1209&params={s_userId:'1'}");
		}
	}
	function closex() {    
