...
<div title="<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"待入职树")%>" region="west" style="float:left;overflow:scroll;width:220px;height:100%" bodyStyle="overflow:hidden;" showHeader="false"
	showFilter="false"
	class="sub-sidebar" allowResize="false">
	<div class="form-control" id="jsp" style="height:100%;padding:5px;">
		<ul id="tree_emp_newEntry_tree" class="ztree" 
parentField="ORG_PARENT_ID" >
		</ul>
	</div>
	</div>
	...
	<iframe id="main" frameborder="0" name="main"  width="100%" border="0" scroll="yes" style="margin-top: 0px;" ></iframe>
	</div>
</div>
</body>
...
var setting = {
			...
			async: {
				enable:true, // 需要异步加载
				type:"get",
				url:"${pageContext.request.contextPath }/form/queryTreeSolution.action?formId=2755&params={s_userId:'1'}",//ajax的请求地址
		...
		var paramtabs_map = {N: [{title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"新员工入职树")%>', url:'<%= basePath%>form/toForm.action?formId=2779'}]}		
//数据过滤  childNodes其实是ajax返回的数据
		function filter(treeId, parentNode, childNodes) {
				if (!childNodes) return null;
					for (var i=0, l=childNodes.length; i<l; i++) {
						childNodes[i].name = childNodes[i].ORG_ABBREVIATION.replace(/\.n/g, '.');
					}
					return childNodes;
				}
		...
		function onAsyncSuccess(event, treeId, treeNode){
				...
				var zTree = $.fn.zTree.getZTreeObj("tree_emp_newEntry_tree");
		...
		//zTree 初始化方法
			$.fn.zTree.init($("#tree_emp_newEntry_tree"), setting);
...
var ORG_ABBREVIATION = node.ORG_ABBREVIATION ? node.ORG_ABBREVIATION.toLowerCase() : "";
	...
	function GetData(){
		if(_treegridselectedrows!=null){
			...
			var selectTree = nui.get('tree_emp_newEntry_tree');  
	...
	function refreshNode() {
		if(!"true".equals("false")){
			var tree = nui.get("tree_emp_newEntry_tree");
			tree.load("${pageContext.request.contextPath }/form/queryTreeSolution.action?formId=2755&params={s_userId:'1'}");
		}
	}
