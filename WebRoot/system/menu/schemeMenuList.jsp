    ...
    <div id="grid_org_list_filter">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"引入菜单")%> onclick="introduceMenu()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"修改")%> onclick="editMenu()"/>
			</div>
		</div>
...
$(function() {
    ...
    function querys() {
        $("#scheme_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/SysMenuList/querySchemeMenuList.action',
                ...
                formatter: function (value, row, index) {
                    if(value==undefined){
                        value='-';
                    	  ...
                    	  return "<a  class=\"eli w50\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"sysSystemSchemeId\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"方案ID\">"+value+"</a>";
                }
            },
            {
...
function introduceMenu(){
	...
	var url='/myehr/system/menu/menuManager1.jsp?schemeId='+schemeId;
	
	parent.layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"引入菜单")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function editMenu(){
	...
	var url='/myehr/system/menu/schemeMenuedit.jsp?schemeMenuId='+select.schemeMenuId;
