	...
	<div style="margin-top:5px;">
		...
		<div class="BTNGROUP_1041" style="margin:0 auto; display:inline-block;height: 35px;">
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"增加")%> onclick="saveMenuIcon()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"修改")%> onclick="editIcon()"/>
			</div>
			<div style="margin-right:10px;display:inline-block">
			    <input type="button" id="addorg_list"  class="btn btn-primary" value=<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"删除")%> onclick="deleteIcon()"/>
			</div>
		</div>
...
$(function() {
    ...
    function querys_1041() {
        $("#org_list_List").bootstrapTable({
            url :'${pageContext.request.contextPath }/SysMenuList/queryAllSysMenuIconList.action',
            ...
            {
                title : '<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"图标ID")%>',
                field : 'iconId',
                align : 'center',
                visible : false,
                valign : 'left',
                formatter: function (value, row, index) {
                    ...
                    return "<a  class=\"eli w100\" style=\"font-size:14px;text-decoration:none;color:black;width:200px;\" name=\"iconId\" data-type=\"text\" data-pk=\""+row.Id+"\" data-title=\"图标ID\">"+value+"</a>";
                }
            },
			 {
...
function saveMenuIcon(){
	var url='/myehr/system/icon/iconedit.jsp';
	
	layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"增加图标")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function editIcon(){
	...
	var url='/myehr/system/icon/iconedit.jsp?iconId='+select.iconId;
	
	layer.open({
		type: 2,
		title:'<%=LangUtil.getLangByCode((String)session.getAttribute("langType"),"修改图标")%>',
		shadeClose: true,
		shade: 0.8,
		maxmin:true,
		zIndex:9999,
...
function deleteIcon(){
	...
	if(selects!=null && selects.length>0){
		for(var i=0;i<selects.length;i++){
			selectId+=selects[i].iconId+",";
		}
	}else {
		alert('请选择一行数据');return ;
	}

	var url='/myehr/SysMenuList/deleteIcon.action?iconId='+selectId;
	$.ajax({
			url:url,
			type:'post',
